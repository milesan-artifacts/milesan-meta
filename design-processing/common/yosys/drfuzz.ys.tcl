...
if { [info exists ::env(FSM_ONEHOT)] }      { set FSM_ONEHOT $::env(FSM_ONEHOT) }           else { set FSM_ONEHOT 0} 
if { [info exists ::env(SAT)] }      { 
    set SAT $::env(SAT);
    if { [info exists ::env(SAT_QUERY_FILE)]} {set SAT_QUERY_FILE $::env(SAT_QUERY_FILE)} else {puts "Please set SAT_QUERY_FILE variable when SAT is enabled."; exit 1}
    # if { [info exists ::env(TB)]} {set TB $::env(TB)} else {puts "Please set TB variable when SAT is enabled."; exit 1}
    } else { set SAT 0} 
...
if { $SAT == 1 } {
    # yosys read_verilog -DSTOP_COND=0 -defer -sv $TB
    yosys clk2fflogic
    yosys memory -nordff
    set fd [open $SAT_QUERY_FILE]
    set SAT_QUERY [string map {"\n" " "} [read $fd]]
    close $fd
    yosys flatten
