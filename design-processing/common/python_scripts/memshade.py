...
# Assume that a memory has at least 8 entries.
MIN_ADDR_WIDTH = 3
...
def __analyze_output_validity_and_latency(output) -> bool:
    ...
    for cycle_id, output in enumerate(seq_of_all_outputs):
        ...
        if is_0b11_met and output == 0b10:
            return True, cycle_id-LATENCY_OFFSET
...
def __analysis_exec_worker(module_name, input_seq_id, input_seq):
    ...
    is_success, latency = __analyze_output_validity_and_latency(output_stdout)
    return is_success, latency

# @param module_content_post_yosys_lines includes the functions
def __analyze_sram_roles_and_latency(module_name: str, module_content_post_yosys_lines: str):
        ...
        for curr_clock_signal in clock_signals:
            for line_id in range(len(module_content_post_yosys_lines)):
                if "always_ff" in module_content_post_yosys_lines[line_id] and f"posedge {curr_clock_signal}" in module_content_post_yosys_lines[line_id]:
                    ...
                    if line_id < len(module_content_post_yosys_lines) and module_content_post_yosys_lines[line_id].lstrip().startswith("if (!"):
    ...
    assert curr_candidate_roles, f"No candidate roles were found for module {module_name}"
    ...
    # TODO Remove
    curr_candidate_roles = [[PortRole.INPUT_DATA, PortRole.READ_ADDRESS_SIGNAL, PortRole.WRITE_ADDRESS_SIGNAL, PortRole.ENABLE, PortRole.WRITE_ENABLE, PortRole.WRITE_MASK]]
    ...
    candidate_roles_found_prefilter_selectors, candidate_roles_found_prefilter_selectors_and_latencies = zip(*candidate_roles_found_prefilter_selectors_and_latencies)
    ...
    return role_dict, latency, signal_dict['clock_signals'], signal_dict['output_signal_and_width']
...
def instrument_sram(module_name: str, module_lines: list):
...
def __list_module_contents(sv_content):
    ...
    while True:
        ...
        for line_id, line in enumerate(curr_module_content_incl_functions.split('\n')):
            if re.match(r'\s*function\s+.*', line):
                ...
                line_ids_to_remove.append(line_id)
        curr_module_content = '\n'.join([line for line_id, line in enumerate(curr_module_content_incl_functions.split('\n')) if line_id not in line_ids_to_remove])

        module_contents.append(curr_module_content)
        module_contents_incl_functions.append(curr_module_content_incl_functions)
        start_idx = end_idx

    return module_contents, module_contents_incl_functions
...
def __module_instrumentation_worker(module_name, module_content):
    module_content_lines = module_content.split('\n')
    return '\n\n' + '\n'.join(instrument_sram(module_name, module_content_lines)) + '\n\n'

def instrument_all_memories_in_design(sv_content):
    all_module_contents, all_module_contents_incl_functions = __list_module_contents(sv_content)
...
# def instrument_design()

# if __name__ == '__main__':
#     with open(f"{MODULE_NAME}.sv", "r") as f:
#         module_content_post_yosys_lines = f.readlines()

#     module_content_post_yosys_lines = top_instrument_sram(module_content_post_yosys_lines)

#     with open(f"{MODULE_NAME}_instrumented.sv", "w") as f:
#         f.writelines(module_content_post_yosys_lines)

# # FUTURE: Check that there is no forwarding of the input data
