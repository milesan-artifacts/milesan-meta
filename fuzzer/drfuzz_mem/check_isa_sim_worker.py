...
def check_isa_sims(design_name: str, num_cores: int, total_tests: int, seed_offset: int, timeout: int = None, seeds = None):
        ...
        if total_tests > 0:
            ...
            while True:
                try:
                    ...
                    print(e)
                process_instance_id += 1
        return seed_to_fail_type_dict, total_finished_tests
    ...
    # supress backtrace when interrupting 
    pool = mp.Pool(processes=num_workers,  initializer=signal.signal, initargs=(signal.SIGINT, signal.SIG_IGN))
    ...
    try:
        ...
        print(f"Iterrupted after finishing {total_finished_tests} threads. Exiting.")
        pool.terminate()
        return seed_to_fail_type_dict,total_finished_tests
