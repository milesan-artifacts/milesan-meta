...
from milesan.cfinstructionclasses_taint import RegdumpInstruction_taint, RDInstruction_taint
...
from milesan.registers import INTREG_ABINAMES,MAX_32b
import subprocess
import time
...
def check_isa_sim_taint(design_name: str,seed: int, generate_fuzzerstate: bool = True, fuzzerstate = None):   
    ...
    fuzzerstate.write_imm_taint_to_mem() # Write the immediate taints from the program code to the imem.
    fuzzerstate.dump_memview_taint()
    try:
        ...
        if not SKIP_RTL:
            regstream_rtl_val, regstream_rtl_val_taint = regstream_rtl
        ...
        for bb_id, bb_instrs in enumerate(fuzzerstate.instr_objs_seq):
            for next_instr in bb_instrs:
                ...
                if DO_DOUBLECHECK_SIM:
                    ...
                    if isinstance(next_instr, RegdumpInstruction_taint):
                        ...
                        if TAINT_EN:
                            try:
                                next_instr.check_regs_taint(regstream_rtl_val_taint[regdump_idx]) # check value before executing instruction
                            except AssertionError as e:
        ...
        if SKIP_RTL:
            return fuzzerstate
        final_regvals, final_regvals_taint = final_regvals_rtl
        for id in range(1,fuzzerstate.num_pickable_regs):
            value = final_regvals[id]
            value_taint = final_regvals_taint[id]
            ...
            if mismatch:
                ...
                if isinstance(last_instr, EPCWriterInstruction) and last_instr.csr_instr.csr_id == CSR_IDS.SEPC:
                    ...
                    pass
                elif "ibex" in fuzzerstate.design_name and mismatch[1]^mismatch[2] == SPIKE_STARTADDR: ## some problem with ibex and the spike offset disappearing in rtl
                    pass
                elif not IGNORE_VALUE_MISMATCH and not is_tolerate(design_name, last_instr):
            ...
            if TAINT_EN:
                mismatch = fuzzerstate.intregpickstate.regs[id].check_taint(value_taint)
        ...
        if DUMP_MCYCLES:
            ...
            mcycle_taint = final_regvals_taint[MAX_NUM_PICKABLE_REGS-1]
            
            if TAINT_EN and mcycle_taint:
                raise MismatchError(f"(RTL) MCYCLE CSR got tainted: {hex(mcycle)}, {hex(mcycle_taint)}.\n\t Taint sources are {[p.name for p in fuzzerstate.taint_source_privs]}, taint sinks are {[p.name for p in fuzzerstate.taint_sink_privs]}.", fail_type=FailTypeEnum.TAINT_MISMATCH)
