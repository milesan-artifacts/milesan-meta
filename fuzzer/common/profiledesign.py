...
from milesan.cfinstructionclasses_taint import RegImmInstruction_taint, ImmRdInstruction_taint, R12DInstruction, CSRImmInstruction
...
from milesan.registers import MAX_32b, MAX_64b, MAX_20b, MAX_12b, INTREG_ABINAMES
...
def __gen_ct_profiling_snippet(design_name, test_instr_str):
    ...
    fuzzerstate.instr_objs_seq[-1].append(ImmRdInstruction_taint(fuzzerstate,"lui", 1, imm, imm, is_rd_nonpickable_ok=False))
    ...
    fuzzerstate.instr_objs_seq[-1].append(RegImmInstruction_taint(fuzzerstate,"addi", 1, 1, imm, imm, is_rd_nonpickable_ok=False))
    ...
    if test_instr_str in RegImmInstruction.authorized_instr_strs:
        test_instr = RegImmInstruction_taint(fuzzerstate,test_instr_str, 0, 1, imm, imm, is_rd_nonpickable_ok=False)
    elif test_instr_str in ImmRdInstruction.authorized_instr_strs:
        test_instr = ImmRdInstruction_taint(fuzzerstate,test_instr_str, 0, imm, imm, is_rd_nonpickable_ok=False)
...
def __check_instr_is_ct(design_name: str, test_instr_str):
    ...
    if int(regdumps[0]['value_taint'],16) == 0:
...
def profile_get_medeleg_mask(design_name: str):
    ...
    if "picorv32" in design_name or 'ibex' in design_name:
...
def profile_get_asid_mask(design_name: str):
    ...
    if 'ibex' in design_name:
        PROFILED_ASID_MASK = 0
        return 0 # This design does not support asid
