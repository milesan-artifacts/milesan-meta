...
class CSRFile():
    ...
    def __init__(self, fuzzerstate):
        from common.profiledesign import PROFILED_MEDELEG_MASK, PROFILED_ASID_MASK
        if PROFILED_MEDELEG_MASK is None and PROFILED_ASID_MASK is None:
    ...
    def restore_state(self, last_saved_state):
        for reg, val_val_taint in zip(self.regs.values(), last_saved_state):
            reg.set_val(val_val_taint[0])
            reg.set_val_taint(val_val_taint[1])

    def save_curr_state(self):
        return copy([(r.get_val(), r.get_val_taint()) for r in self.regs.values()]) # no deepcopy
