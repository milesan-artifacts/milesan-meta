...
from milesan.cfinstructionclasses_taint import JALInstruction_taint, RegImmInstruction_taint, JALRInstruction_taint, ImmRdInstruction_taint, RegImmInstruction_taint, R12DInstruction_taint, BranchInstruction_taint
...
def _save_ctx_and_jump_to_pillar_specific_instr(fuzzerstate, index_first_bb_to_consider: int, index_first_instr_to_consider: int):
    ...
    if TAINT_EN and DO_ASSERT:
        ...
        dumpedvals_in_situ, dumpedvals_taint = fuzzerstate_cpy.get_regdumps_from_reqs(ctx_regdump_reqs, True, None, False, True)
        ...
        for idx, (in_situ_d, in_situ_d_taint, spike_d) in enumerate(zip(dumpedvals_in_situ, dumpedvals_taint, dumpedvals)):
            assert in_situ_d == spike_d or in_situ_d_taint == 0, f"Mismatch between in-situ simulation and spike ctx dump req at addr {hex(ctx_regdump_reqs[idx][0])} for reg ID {ctx_regdump_reqs[idx][2]}: Traceback {filter_reg_traceback(INTREG_ABINAMES.index(ctx_regdump_reqs[idx][2]),ctx_regdump_reqs[idx][0],fuzzerstate,spike_d).get_str()}: {hex(in_situ_d)} != {hex(spike_d)}, ({hex(in_situ_d_taint)})"
    ...
    for _ in range(num_stores_found):
        ...
        for byte_id in range(storenumbytes[curr_id_in_storenumbytes]):
            ...
            val_taint = (dumpedvals_taint[curr_id_in_dumpedvals+1] >> (8*byte_id)) & 0xFF
            saved_stores[addr] = val
            saved_stores_taint[addr] = val_taint # We only care for the taints of the stores, not CSRs as those are untainted by construction.
            # print(f"instr: {insts[curr_id_in_dumpedvals].get_str()}: {hex(val)} ({hex(val_taint)}) -> {hex(addr)}")
    ...
    saved_regvals_taint  = dumpedvals_taint[curr_id_in_dumpedvals:curr_id_in_dumpedvals+fuzzerstate.num_pickable_regs]
    ...
    new_jump = JALInstruction_taint(fuzzerstate,"jal", 0, fuzzerstate.ctxsv_bb_base_addr - 4*(len(fuzzerstate.instr_objs_seq[fuzzerstate.last_bb_id_before_ctx_saver])-1)) # NO_COMPRESSED
...
def gen_reduced_elf(fuzzerstate, max_bb_id_to_consider: int, max_instr_id_except_cf: int = None, index_first_bb_to_consider: int = 1, index_first_instr_to_consider: int = 0, keep_dead_code_in_memory: bool = True):
    ...
    if DO_ASSERT:
        if isinstance(test_fuzzerstate.instr_objs_seq[max_bb_id_to_consider][-1], JALInstruction_taint):
    ...
    if max_instr_id_except_cf <  len(test_fuzzerstate.instr_objs_seq[max_bb_id_to_consider])-2:
        ...
        new_jal = JALInstruction_taint(test_fuzzerstate, "jal", 0, test_fuzzerstate.final_bb_base_addr-last_instr.paddr+SPIKE_STARTADDR)
...
def is_mismatch_taint(test_fuzzerstate,quiet = False):
...
def _reduce_taint(fuzzerstate,quiet:bool=False):
    assert REDUCE_TAINT
    tainted_addresses = [i for i,j in fuzzerstate.memview.data_taint.items() if j != 0]
    ...
    while left_bound < right_bound: # Assumes we leak a single word.
        ...
        for i in range(0, left_bound):
            test_fuzzerstate.memview.data_taint[tainted_addresses[i]] = 0
    ...
    for addr_idx in range(0, len(tainted_addresses)): # start with right side so the left bound does not change
        if addr_idx != left_bound:
            test_fuzzerstate.memview.data_taint[tainted_addresses[addr_idx]] = 0
...
def _try_flatten_cf(fuzzerstate, failing_bb, failing_instr, pillar_bb, pillar_instr_id, quiet: bool = False):
    ...
    n_jal_instrs = sum([1 for bb in flat_fuzzerstate.instr_objs_seq[1:] if isinstance(bb[-1], (JALRInstruction_taint,JALInstruction_taint))])
    ...
    for bb_id, bb in enumerate(flat_fuzzerstate.instr_objs_seq):
        ...
        if isinstance(last_instr, (JALInstruction_taint,JALRInstruction_taint)):
            ...
            lui_instr = ImmRdInstruction_taint(flat_fuzzerstate,'lui',last_instr.rd,lui_imm)
            lui_instr.paddr = addr_flat_instrs + 4*len(new_flat_instrs) + SPIKE_STARTADDR
            new_flat_instrs += [lui_instr]
            addi_instr = RegImmInstruction_taint(flat_fuzzerstate, 'addi', last_instr.rd, last_instr.rd, addi_imm)
            addi_instr.paddr = addr_flat_instrs + 4*len(new_flat_instrs) + SPIKE_STARTADDR
            new_flat_instrs += [addi_instr]
            add_instr = R12DInstruction_taint(flat_fuzzerstate, 'add', last_instr.rd, last_instr.rd, RELOCATOR_REGISTER_ID)  # add the spike offset again
    ...
    jal_inst = JALInstruction_taint(flat_fuzzerstate, "jal", 0, flat_fuzzerstate.final_bb_base_addr - 4*(len(new_flat_instrs)) - addr_flat_instrs)
    ...
    jal_inst = JALInstruction_taint(flat_fuzzerstate, "jal", 0, flat_fuzzerstate.bb_start_addr_seq[1] - (flat_fuzzerstate.ctxsv_bb_start_addr_seq[0] + 4*flat_fuzzerstate.ctxsv_bb_jal_instr_id))
...
def _turn_sandwich_instructions_into_nops(fuzzerstate, failing_bb_id: int, failing_instr_id: int, pillar_bb_id: int, pillar_instr_id: int, fault_from_prev_bb: bool, quiet: bool = False):
    ...
    if pillar_bb_id == failing_bb_id:
        ...
        for instr_id in range(pillar_instr_id, failing_instr_id+1):
            ...
            nop_instr =  RegImmInstruction_taint(fuzzerstate,"addi", 0, 0, 0)
...
def reduce_program(memsize: int, design_name: str, randseed: int, nmax_bbs: int, authorize_privileges: bool, quiet: bool = False, target_dir: str = None, hint_left_bound_bb: int = None, hint_right_bound_bb: int = None, hint_left_bound_instr: int = None, hint_right_bound_instr: int = None, hint_left_bound_pillar_bb: int = None, hint_right_bound_pillar_bb: int = None, hint_left_bound_pillar_instr: int = None, hint_right_bound_pillar_instr: int = None, check_pc_spike_again: bool = False):
    ...
    for block_id, instr_id in fuzzerstate.fpuendis_coords:
        ...
        fuzzerstate.instr_objs_seq[block_id][instr_id] = RegImmInstruction_taint(fuzzerstate,"addi", 0, 0, 0)
    ...
    n_instr_before_reduction = sum([len(i) for i in fuzzerstate.instr_objs_seq[1:]])
    ...
    # n_instr_after_find_failing_bb = sum([len(i) for i in fuzzerstate.instr_objs_seq[1:]])
    ###
    ...
    # n_instr_after_find_failing_instr_in_bb = sum([len(i) for i in fuzzerstate.instr_objs_seq[1:]])
    ...
    if FIND_PILLARS:
        ...
        # n_instr_after_find_pillar_bb = sum([len(i) for i in fuzzerstate.instr_objs_seq[1:]])
        ###
        ...
        if FIND_PILLAR_INSTRUCTION and pillar_bb_id>0:
            ...
            # n_instr_after_find_pillar_instr = sum([len(i) for i in fuzzerstate.instr_objs_seq[1:]])
    ...
    n_instr_after_reduction = sum([len(i) for i in fuzzerstate.instr_objs_seq[1:]])
    ...
    if REDUCE_DEAD_CODE:
        n_spec_instr_before_reduce_dead_code = len(final_fuzzerstate.spec_instr_objs_seq)
        ...
        n_spec_instr_after_reduce_dead_code = len(final_fuzzerstate.spec_instr_objs_seq)
    ...
    if REDUCE_TAINT:
        ...
        try:
            n_tainted_bytes_before_reduce_taint = len([i for i,j in final_fuzzerstate.memview.data_taint.items() if j != 0])
        ...
        n_tainted_bytes_after_reduce_taint = len([i for i,j in final_fuzzerstate.memview.data_taint.items() if j != 0])
    ...
    ret_dict["n_instr_before_reduction"] = n_instr_before_reduction
    ret_dict["n_instr_after_redction"] = n_instr_after_reduction
    ...
    if FIND_PILLARS: 
        ...
        # ret_dict["n_instr_after_find_pillar_bb"] = n_instr_after_find_pillar_bb
    ...
    if FIND_PILLAR_INSTRUCTION:
        ...
        # ret_dict["n_instr_after_find_pillar_instr"] = n_instr_after_find_pillar_instr
    ...
    if REDUCE_TAINT:
        ...
        ret_dict["n_tainted_bytes_before_reduce_taint"] = n_tainted_bytes_before_reduce_taint
        ret_dict["n_tainted_bytes_after_reduce_taint"] = n_tainted_bytes_after_reduce_taint
...
def _count_instructions(fuzzerstate, max_bb, max_instr, min_bb, min_instr):
    nop_instr_bytecode =  RegImmInstruction_taint(fuzzerstate,"addi", 0, 0, 0).gen_bytecode_int(USE_SPIKE_INTERM_ELF)
    n_nops = 0
    n_non_nop_insts = 0
...
def _check_leakage_without_cellift(fuzzerstate, n_elfs:int , multithread:bool, max_bb_id_to_consider, max_instr_id_except_cf, index_first_bb_to_consider, index_first_instr_to_consider):
    for i in range(n_elfs):
        test_fuzzerstate = deepcopy(fuzzerstate)
        for addr, val_taint in test_fuzzerstate.memview.data_taint.items():
            if val_taint:
