...
def li_doubleword(value, rd, tmp, fuzzerstate, is_rd_nonpickable_ok: bool = False):
    from milesan.cfinstructionclasses_taint import R12DInstruction_taint, ImmRdInstruction_taint, RegImmInstruction_taint
    ...
    instrs.append(ImmRdInstruction_taint(fuzzerstate, "lui", tmp, lui_imm, is_rd_nonpickable_ok=is_rd_nonpickable_ok))
    instrs.append(RegImmInstruction_taint(fuzzerstate, "addi", tmp, tmp, addi_imm, is_rd_nonpickable_ok=is_rd_nonpickable_ok))
    #clear the top 32 bits
    instrs.append(R12DInstruction_taint(fuzzerstate, "and", tmp, tmp, RDEP_MASK_REGISTER_ID))
    #load the next 32 bits
    lui_imm_2, addi_imm_2 = li_into_reg(imm_63_to_31, False)
    instrs.append(ImmRdInstruction_taint(fuzzerstate, "lui", rd, lui_imm_2, is_rd_nonpickable_ok=is_rd_nonpickable_ok))
    instrs.append(RegImmInstruction_taint(fuzzerstate, "addi", rd, rd, addi_imm_2, is_rd_nonpickable_ok=is_rd_nonpickable_ok))
    instrs.append(RegImmInstruction_taint(fuzzerstate, "slli", rd, rd, 32, is_rd_nonpickable_ok=is_rd_nonpickable_ok))
    #coalesce the result
    instrs.append(R12DInstruction_taint(fuzzerstate, "or", rd, rd, tmp, is_rd_nonpickable_ok=is_rd_nonpickable_ok))
