...
class FuzzerState:
    ...
    def append_and_execute_instr(self, instr, insert_regdump: bool = INSERT_REGDUMPS):            
        ...
        if insert_regdump:
            ...
            if has_taint_trace(instr) and instr.rd < MAX_NUM_PICKABLE_REGS and self.intregpickstate.regs[instr.rd].fsm_state == IntRegIndivState.FREE:
                store_instr = RegdumpInstruction_taint(self,"sd" if self.is_design_64bit else "sw", REGDUMP_REGISTER_ID, instr.rd,0,-1)
                ...
                n_bytes += 4
                if INSERT_FENCE:
                    fence_instr = SpecialInstruction_taint(self,"fence")
    ...
    def write_imm_taint_to_mem(self):
        for bb_instrs in self.instr_objs_seq:
            for next_instr in bb_instrs:
                if TAINT_EN and isinstance(next_instr, (ImmRdInstruction_taint, RegImmInstruction_taint, BranchInstruction_taint)):
                    next_instr.write_taint() # Write tainted bytecode to instruction memory if taint is enabled.
    ...
    def dump_instructions_taint(self):
        insts = {}
            ...
            for instr_obj in bb_instrs:
                if instr_obj.injectable:
                    insts[bb_id] += [{"bytecode": instr_obj.gen_bytecode_int(is_spike_resolution=True),
                                    "bytecode_taint": instr_obj.gen_bytecode_int_taint(is_spike_resolution=True),
                                    "addr": instr_obj.paddr,
                                    "type": instr_obj.instr_type.name, 
                                    "str": instr_obj.instr_str,
                                    "bb_id": bb_id}]
    
    def dump_memview_taint(self, path: str = None):
    ...
    def get_regdumps_from_reqs(self, regdump_reqs, is_spike_resolution, final_address, dump_final_reg_vals, skip_placeholder):
                ...
                while regdump_idx < len(regdump_reqs) and  (next_instr.vaddr if USE_MMU else next_instr.paddr) == regdump_reqs[regdump_idx][0]: # there could be multiple dumps for this address
                    ...
                    if is_floatdump:
                        ...
                        if reg_id in CSR_ABI_NAMES + ["priv"]: # We dont dump CSR values or privileges here since we don't allow them to be tainted for now.
                            ...
                            regdumps_taint += [self.intregpickstate.regs[reg_id].get_val_taint()]
        ...
        if not dump_final_reg_vals:
            self.reset_states()
            return (regdumps, regdumps_taint)
        # Retrieve the final register values
        final_intreg_vals = []
        final_intreg_vals_taint = []
        for reg_id in range(self.num_pickable_regs):
            final_intreg_vals += [self.intregpickstate.regs[reg_id].get_val()]
            final_intreg_vals_taint += [self.intregpickstate.regs[reg_id].get_val_taint()]

        self.reset_states()
        return (regdumps, regdumps_taint),((final_intreg_vals, final_intreg_vals_taint), (None, None))
    ...
    def compute_context_stats(self, print_stats: bool = PRINT_PRIV_STATS):
        ...
        for bb_instrs in self.instr_objs_seq:
            for next_instr in bb_instrs:
                if isinstance(next_instr, RDInstruction_taint) and next_instr.writeback_trace["in-situ"]:
    ...
    def compute_taint_stats(self):
        ...
        for bb_id, bb_instrs in enumerate(self.instr_objs_seq):
            for next_instr in bb_instrs:
                ...
                stats = {
                    "n_tainted_regs_ratio": sum([self.intregpickstate.regs[i].get_val_taint() != 0 for i in range(self.num_pickable_regs)])/self.num_pickable_regs,
                    "all_reg_taints": [self.intregpickstate.regs[i].get_val_taint() for i in range(self.num_pickable_regs)],
                    ...
                    "rs1_value_taint": None if not hasattr(next_instr, "rs1") else self.intregpickstate.regs[next_instr.rs1].get_val_taint(),
                    "rs2_value_taint": None if not hasattr(next_instr, "rs2") else self.intregpickstate.regs[next_instr.rs2].get_val_taint(),
                    "imm_value": None if not hasattr(next_instr, "imm") else next_instr.imm,
                    "imm_value_taint": None if not hasattr(next_instr, "imm_taint") else next_instr.imm_taint,
                    "rd_value_before_exec": None if not hasattr(next_instr, "rd") else self.intregpickstate.regs[next_instr.rd].get_val(),
                    "rd_value_taint_before_exec": None if not hasattr(next_instr, "rd") else self.intregpickstate.regs[next_instr.rd].get_val_taint(),
                ...
                stats["rd_value_taint_after_exec"] =  None if not hasattr(next_instr, "rd") else self.intregpickstate.regs[next_instr.rd].get_val_taint()
    ...
    def blacklist_gadget_addr(self, addr, va_layout, priv):
        ...
        # account for superposition 
            ...
            for reg_id in range(self.intregpickstate.num_pickable_regs):
                reg_taint = curr_state[-1][reg_id].get_val_taint() | self.taint_source_transient_addrs_regs[addr][-1][reg_id].get_val_taint()
                ...
                self.taint_source_transient_addrs_regs[addr][-1][reg_id].set_val_taint(reg_taint) # or both taints
