...
def gen_next_instrstr_from_isaclass(isaclass: ISAInstrClass, fuzzerstate) -> str:
    ...
    if isaclass in [ISAInstrClass.MEM]:
        if TAINT_EN and fuzzerstate.privilegestate.privstate in fuzzerstate.taint_source_privs and fuzzerstate.intregpickstate.get_num_tainted_regs_in_state(IntRegIndivState.FREE) == 0:
