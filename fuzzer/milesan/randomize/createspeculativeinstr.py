...
def _create_spectre_gadget_instrobjs(fuzzerstate, instr_str):
    ...
    if va_layout == -1: # We don't need 64bit values in bare.
        ...
        instr_objs += [
                SpeculativeInstructionEncapsulator(fuzzerstate, RegImmInstruction_taint(fuzzerstate,"andi",rs2,rs2,0xFF)), # mask out single byte
                SpeculativeInstructionEncapsulator(fuzzerstate, R12DInstruction_taint(fuzzerstate, "add", rd, rd, rs2)), # add tainted offset to rd
                SpeculativeInstructionEncapsulator(fuzzerstate, IntStoreInstruction_taint(fuzzerstate,store_str,rd,rs1, 0x0, None)) # store taitned reg at tainted offset
            ]
        ...
        if priv_level in fuzzerstate.taint_sink_privs:
            instr_objs += [SpeculativeInstructionEncapsulator(fuzzerstate, IntLoadInstruction_taint(fuzzerstate, load_str, rs2, rd1, 0x0, None))]  # Speculatively load tainted data into rs2 if we are in a privelege mode that cannot access tainted data.
        
        instr_objs += [
                SpeculativeInstructionEncapsulator(fuzzerstate, RegImmInstruction_taint(fuzzerstate,"andi",rs2,rs2,0xFF)), # mask out single byte
                SpeculativeInstructionEncapsulator(fuzzerstate, R12DInstruction_taint(fuzzerstate, "add", rd2, rd2, rs2)), # add tainted offset to rd2
                SpeculativeInstructionEncapsulator(fuzzerstate, IntStoreInstruction_taint(fuzzerstate,store_str,rd2,rs2,0x0, None)) # store taitned reg at tainted offset
            ]
