...
from params.fuzzparams import NUM_MIN_FREE_INTREGS, NUM_MIN_UNTAINTED_INTREGS, REG_FSM_WEIGHTS, NONTAKEN_BRANCH_INTO_RANDOM_DATA_PROBA, PROBA_NEW_SATP_NOT_USED, NUM_MAX_PRODUCED0_REGS, NUM_MAX_PRODUCED1_REGS, TAINT_NONTAKEN_BRANCH_IMM, TAINT_IMMRD_IMM, P_LOAD_TAINT, DISABLE_COMPUTATION_ON_TAINT, TAINT_EN
...
from milesan.cfinstructionclasses_taint import (
    BranchInstruction_taint,
    ImmRdInstruction_taint,
    IntLoadInstruction_taint,
    IntStoreInstruction_taint,
    JALInstruction_taint,
    JALRInstruction_taint,
    PlaceholderConsumerInstr_taint,
    PlaceholderPreConsumerInstr_taint,
    PlaceholderProducerInstr0_taint,
    PlaceholderProducerInstr1_taint,
    R12DInstruction_taint,
    RegImmInstruction_taint,
    SpecialInstruction_taint,
)
...
from milesan.toleratebugs import is_tolerate_ibex_div_ct_violation, is_tolerate_ibex_divu_ct_violation, is_tolerate_ibex_divuw_ct_violation, is_tolerate_ibex_divw_ct_violation
from milesan.toleratebugs import is_tolerate_ibex_rem_ct_violation, is_tolerate_ibex_remu_ct_violation, is_tolerate_ibex_remuw_ct_violation, is_tolerate_ibex_remw_ct_violation, is_tolerate_ibex_mul_ct_violation, is_tolerate_ibex_mulh_ct_violation, is_tolerate_ibex_mulhsu_ct_violation, is_tolerate_ibex_mulhu_ct_violation
...
def gen_random_imm_taint(instr_str: str, fuzzerstate):
...
def is_tolerate_R12DInstruction(instr_str, fuzzerstate):
    ...
    or "ibex" in fuzzerstate.design_name and \
        (not is_tolerate_ibex_divuw_ct_violation() and instr_str == "divuw" \
        or not is_tolerate_ibex_divw_ct_violation() and instr_str == "divw" \
        or not is_tolerate_ibex_div_ct_violation() and instr_str == "div" \
        or not is_tolerate_ibex_divu_ct_violation() and instr_str == "divu" \
        or not is_tolerate_ibex_remuw_ct_violation() and instr_str == "remuw" \
        or not is_tolerate_ibex_remw_ct_violation() and instr_str == "remw" \
        or not is_tolerate_ibex_rem_ct_violation() and instr_str == "rem" \
        or not is_tolerate_ibex_remu_ct_violation() and instr_str == "remu" 
        or not is_tolerate_ibex_mul_ct_violation() and instr_str == "mul" \
        or not is_tolerate_ibex_mulh_ct_violation() and instr_str == "mulh" \
        or not is_tolerate_ibex_mulhsu_ct_violation() and instr_str == "mulhsu" \
        or not is_tolerate_ibex_mulhu_ct_violation() and instr_str == "mulhu") \
...
def _create_R12DInstruction(instr_str: str, fuzzerstate, iscompressed: bool):
    ...
    return R12DInstruction_taint(fuzzerstate, instr_str, rd, rs1, rs2, iscompressed)
...
def _create_ImmRdInstruction(instr_str: str, fuzzerstate, iscompressed: bool):
    ...
    if TAINT_EN and fuzzerstate.privilegestate.privstate in fuzzerstate.taint_source_privs and "auipc" not in instr_str and not DISABLE_COMPUTATION_ON_TAINT:
        n_free_untainted_regs = fuzzerstate.intregpickstate.get_num_untainted_regs_in_state(IntRegIndivState.FREE)
        if n_free_untainted_regs < NUM_MIN_UNTAINTED_INTREGS: # There's too much taint, remove some.
            ...
            if TAINT_IMMRD_IMM:
                imm_taint = gen_random_imm_taint(instr_str, fuzzerstate)
            else:
                imm_taint = 0
            rd = fuzzerstate.intregpickstate.pick_untainted_int_outputreg_nonzero()
    else:
    ...
    if USE_COMPRESSED and instr_str in IS_COMPRESSABLE:
        ...
        if is_compressable and (random.random() < COMPRESS_INSTRUCTION):
            if imm_taint:
                imm_taint = gen_random_imm_taint(instr_str_cmp, fuzzerstate)
            iscompressed = True
            # print(f"compressed {instr_str} into {instr_str_cmp}") #DEBUG
            instr_str = instr_str_cmp

    return ImmRdInstruction_taint(fuzzerstate,instr_str, rd, imm, imm_taint, iscompressed)
...
def _create_RegImmInstruction(instr_str: str, fuzzerstate, iscompressed: bool):
    ...
    if TAINT_EN and fuzzerstate.privilegestate.privstate in fuzzerstate.taint_source_privs and not DISABLE_COMPUTATION_ON_TAINT:
        n_free_untainted_regs = fuzzerstate.intregpickstate.get_num_untainted_regs_in_state(IntRegIndivState.FREE)
        if n_free_untainted_regs < NUM_MIN_UNTAINTED_INTREGS: # There's too much taint, remove some.
            imm_taint = 0 
    ...
    if USE_COMPRESSED and instr_str in IS_COMPRESSABLE:
        ...
        if is_compressable and (random.random() < COMPRESS_INSTRUCTION):
            if imm_taint:
                imm_taint = gen_random_imm_taint(instr_str_cmp, fuzzerstate)    
            iscompressed = True
            # print(f"compressed {instr_str}, {INTREG_ABINAMES[rd]}, {INTREG_ABINAMES[rs1]}, {hex(imm)}, into {instr_str_cmp}") #DEBUG
            instr_str = instr_str_cmp

    return RegImmInstruction_taint(fuzzerstate, instr_str, rd, rs1, imm, imm_taint, iscompressed)
...
def _create_BranchInstruction(instr_str: str, fuzzerstate, curr_addr: int, iscompressed: bool):
    ...
    if TAINT_NONTAKEN_BRANCH_IMM:
        if TAINT_EN and not plan_taken and random.random() < 0.5 and fuzzerstate.privilegestate.privstate in fuzzerstate.taint_source_privs:
            imm_taint = random.randint(0, 1<<curr_param_size)
    ...
    return BranchInstruction_taint(fuzzerstate, instr_str, rs1, rs2, imm, imm_taint, plan_taken, iscompressed)
...
def _create_JALInstruction(instr_str: str, fuzzerstate, curr_addr: int, iscompressed: bool):
    ...
    if USE_COMPRESSED and len(fuzzerstate.instr_objs_seq) > 1 and instr_str in IS_COMPRESSABLE: # no compressed in initial block
        ...
        if is_compressable and (random.random() < COMPRESS_INSTRUCTION):
            ...
            # print(f"compressed {instr_str} into {instr_str_cmp}") #DEBUG
            instr_str = instr_str_cmp
    return JALInstruction_taint(fuzzerstate, instr_str, rd, imm, iscompressed)
...
def _create_JALRInstruction(instr_str: str, fuzzerstate, iscompressed: bool, curr_addr: int = None): # curr_addr for compatibility in _create_spectre_gadget_instrobjs 
    ...
    assert not fuzzerstate.intregpickstate.regs[rs1].get_val_taint(), f"rs1 {INTREG_ABINAMES[rs1]} for JALR is tainted!"
    rd = fuzzerstate.intregpickstate.pick_untainted_int_outputreg()
    imm = 0
    ...
    return JALRInstruction_taint(fuzzerstate, instr_str, rd, rs1, imm, producer_id, iscompressed)
...
def _create_SpecialInstruction(instr_str: str, fuzzerstate, iscompressed: bool):
    ...
    rs1 = fuzzerstate.intregpickstate.pick_int_inputreg()
    return SpecialInstruction_taint(fuzzerstate, instr_str, rd, rs1)
...
def _create_IntLoadInstruction(instr_str: str, fuzzerstate, iscompressed: bool):
    if DO_ASSERT:
        ...
        if TAINT_EN and fuzzerstate.privilegestate.privstate in fuzzerstate.taint_source_privs and fuzzerstate.effective_curr_layout in fuzzerstate.taint_source_layouts:
            assert fuzzerstate.intregpickstate.exists_reg_in_state(IntRegIndivState.PAGE_TAINT_ADDR)
        else:
    ...
    if TAINT_EN:
        if fuzzerstate.privilegestate.privstate in fuzzerstate.taint_source_privs and fuzzerstate.effective_curr_layout in fuzzerstate.taint_source_layouts:
            ...
            taint = False
    rs1 = fuzzerstate.intregpickstate.pick_int_reg_in_state(IntRegIndivState.PAGE_TAINT_ADDR if taint else IntRegIndivState.PAGE_ADDR)
    ...
    if USE_COMPRESSED and instr_str in IS_COMPRESSABLE:
        ...
        if is_compressable and (random.random() < COMPRESS_INSTRUCTION):
            ...
            # print(f"compressed {instr_str} into {instr_str_cmp}") #DEBUG
            instr_str = instr_str_cmp
    return IntLoadInstruction_taint(fuzzerstate, instr_str, rd, rs1, imm, None, iscompressed)
...
def _create_IntStoreInstruction(instr_str: str, fuzzerstate, iscompressed: bool):
    ...
    return IntStoreInstruction_taint(fuzzerstate, instr_str, rs1, rs2, imm, None, iscompressed)
...
def create_targeted_producer0_instrobj(fuzzerstate):
    ...
    return [PlaceholderProducerInstr0_taint(fuzzerstate, rd, fuzzerstate.next_producer_id)]
...
def create_targeted_producer1_instrobj(fuzzerstate):
    ...
    return [PlaceholderProducerInstr1_taint(fuzzerstate, rd, fuzzerstate.intregpickstate.get_producer_id(rd))]
...
def create_targeted_consumer_instrobj(fuzzerstate):
    ...
    assert fuzzerstate.intregpickstate.regs[rdep].get_val_taint() == 0
    ...
    if fuzzerstate.is_design_64bit:
        # return [PlaceholderPreConsumerInstr_taint, PlaceholderPreConsumerInstr_taint, PlaceholderConsumerInstr_taint], \
        ...
        return [
            PlaceholderPreConsumerInstr_taint(fuzzerstate, rprod, fuzzerstate.intregpickstate.get_producer_id(rprod), True),
            PlaceholderPreConsumerInstr_taint(fuzzerstate, rdep, fuzzerstate.intregpickstate.get_producer_id(rprod)),
            PlaceholderConsumerInstr_taint(fuzzerstate, rd, rdep, rprod, fuzzerstate.intregpickstate.get_producer_id(rprod))
        ]
    else:
        return [PlaceholderConsumerInstr_taint(fuzzerstate, rd, rdep, rprod, fuzzerstate.intregpickstate.get_producer_id(rprod))]
...
def create_memfsm_instrobjs(fuzzerstate):
    ...
    if not fuzzerstate.intregpickstate.exists_reg_in_state(IntRegIndivState.PAGE_TAINT_ADDR):
        tainted = True
    ...
    if va_layout == -1: # We don't need 64bit values in bare.
        ...
        if tainted:
            if not fuzzerstate.intregpickstate.exists_reg_in_state(IntRegIndivState.PAGE_TAINT_ADDR):
                ...
                rd =  fuzzerstate.intregpickstate.pick_int_reg_in_state(IntRegIndivState.PAGE_TAINT_ADDR)
        else:
        ...
        fuzzerstate.intregpickstate.set_regstate(rd, IntRegIndivState.PAGE_TAINT_ADDR if tainted else IntRegIndivState.PAGE_ADDR, force=True)
        ...
        return [
            ImmRdInstruction_taint(fuzzerstate, "lui", rd, uimm0),
            RegImmInstruction_taint(fuzzerstate, "addi",rd,rd,uimm1),
            R12DInstruction_taint(fuzzerstate, "xor",rd,rd,RELOCATOR_REGISTER_ID)
        ]
        ...
        if fuzzerstate.is_design_64bit:
            ...
            instr_objs = []
            instr_objs.append(ImmRdInstruction_taint(fuzzerstate,"lui", rd, lui_imm))
            instr_objs.append(RegImmInstruction_taint(fuzzerstate,"addi", rd, rd, addi_imm))
