...
from milesan.cfinstructionclasses_taint import TvecWriterInstruction_taint, EPCWriterInstruction_taint, GenericCSRWriterInstruction_taint, SimpleExceptionEncapsulator_taint, CSRRegInstruction_taint, SimpleIllegalInstruction_taint, MisalignedMemInstruction_taint, MstatusWriterInstruction_taint, R12DInstruction_taint, RegImmInstruction_taint, ImmRdInstruction_taint, IntLoadInstruction_taint
...
def pick_illegal_instruction(is_mtvec, fuzzerstate):
    if "vexriscv" in fuzzerstate.design_name and is_tolerate_vexriscv_fpu_disabled() and not fuzzerstate.is_fpu_activated:
        ...
        return SimpleExceptionEncapsulator_taint(fuzzerstate,is_mtvec, None, Float3Instruction('fadd.s', 0, 0, 0, 0, False),  ExceptionCauseVal.ID_ILLEGAL_INSTRUCTION)
    if "vexriscv" in fuzzerstate.design_name and is_tolerate_vexriscv_fpu_leak() and fuzzerstate.is_fpu_activated:
        ...
        return SimpleExceptionEncapsulator_taint(fuzzerstate,is_mtvec, None, CSRRegInstruction_taint(fuzzerstate,'csrrw', rd, rs1, CSR_IDS.FCSR),  ExceptionCauseVal.ID_ILLEGAL_INSTRUCTION)
    ...
    if corrected_simple_illegal_instruction_proba < random.random():
        return SimpleExceptionEncapsulator_taint(fuzzerstate, is_mtvec, None, SimpleIllegalInstruction_taint(fuzzerstate, is_mtvec), ExceptionCauseVal.ID_ILLEGAL_INSTRUCTION)
    ...
    if not fuzzerstate.design_has_fpu or not fuzzerstate.is_fpu_activated \
        ...
        if random.random() < PROBA_PICK_WRONG_FPU * 0.01**fuzzerstate.design_has_fpu:
            ...
            frs1, frs2 = random.randrange(MAX_NUM_PICKABLE_FLOATING_REGS), random.randrange(MAX_NUM_PICKABLE_FLOATING_REGS)
            frd = random.randrange(MAX_NUM_PICKABLE_FLOATING_REGS)
            return SimpleExceptionEncapsulator_taint(fuzzerstate,is_mtvec, None, Float3Instruction(random.choice(Float3Instructions), frd, frs1, frs2, rm, False), ExceptionCauseVal.ID_ILLEGAL_INSTRUCTION)
    ...
    if fuzzerstate.privilegestate.prev_privstate == PrivilegeStateEnum.MACHINE:
        if 'kronos' in fuzzerstate.design_name and not is_tolerate_kronos_readbadcsr() \
            ...
            candidate_instructions = [
                SimpleExceptionEncapsulator_taint(fuzzerstate,is_mtvec, None, SimpleIllegalInstruction_taint(fuzzerstate, is_mtvec), ExceptionCauseVal.ID_ILLEGAL_INSTRUCTION),
            ]
        else:
            candidate_instructions = [
                SimpleExceptionEncapsulator_taint(fuzzerstate,is_mtvec, None, SimpleIllegalInstruction_taint(fuzzerstate, is_mtvec),  ExceptionCauseVal.ID_ILLEGAL_INSTRUCTION),
                SimpleExceptionEncapsulator_taint(fuzzerstate,is_mtvec, None, CSRRegInstruction_taint(fuzzerstate,"csrrw", random.randrange(fuzzerstate.num_pickable_regs), random.randrange(fuzzerstate.num_pickable_regs), CSR_IDS.UNIMP),  ExceptionCauseVal.ID_ILLEGAL_INSTRUCTION),
            ]
    elif fuzzerstate.privilegestate.prev_privstate == PrivilegeStateEnum.SUPERVISOR:
        candidate_instructions = [
            SimpleExceptionEncapsulator_taint(fuzzerstate,is_mtvec, None, PrivilegeDescentInstruction(fuzzerstate, True), ExceptionCauseVal.ID_ILLEGAL_INSTRUCTION),
            SimpleExceptionEncapsulator_taint(fuzzerstate,is_mtvec, None, CSRRegInstruction_taint(fuzzerstate,"csrrw", random.randrange(fuzzerstate.num_pickable_regs), random.randrange(fuzzerstate.num_pickable_regs), random.choice(INTERESTING_CSRS_INACCESSIBLE_FROM_SUPERVISOR)),  ExceptionCauseVal.ID_ILLEGAL_INSTRUCTION),
        ]
    elif fuzzerstate.privilegestate.prev_privstate == PrivilegeStateEnum.USER:
        candidate_instructions = [
            SimpleExceptionEncapsulator_taint(fuzzerstate,is_mtvec, None, random.choice([PrivilegeDescentInstruction(fuzzerstate, True), PrivilegeDescentInstruction(fuzzerstate, False)]), ExceptionCauseVal.ID_ILLEGAL_INSTRUCTION),
            SimpleExceptionEncapsulator_taint(fuzzerstate,is_mtvec, None, CSRRegInstruction_taint(fuzzerstate,"csrrw", random.randrange(fuzzerstate.num_pickable_regs), random.randrange(fuzzerstate.num_pickable_regs), random.choice(INTERESTING_CSRS_INACCESSIBLE_FROM_USER)),  ExceptionCauseVal.ID_ILLEGAL_INSTRUCTION),
        ]
    else:
...
def gen_next_exception_instr_from_instroptype(fuzzerstate, exception_op_type: ExceptionCauseVal):
    ...
    if exception_op_type == ExceptionCauseVal.ID_INSTR_ADDR_MISALIGNED:
        ...
        return SimpleExceptionEncapsulator_taint(fuzzerstate,is_mtvec, None, JALInstruction(fuzzerstate, "jal", 0, misaligned_tgt_addr - jal_addr),exception_op_type)
        ...
        return SimpleExceptionEncapsulator_taint(fuzzerstate,is_mtvec, None, EcallEbreakInstruction(fuzzerstate,"ebreak"),exception_op_type)
    elif exception_op_type == ExceptionCauseVal.ID_LOAD_ADDR_MISALIGNED:
        if DO_ASSERT:
            assert fuzzerstate.intregpickstate.exists_reg_in_state(IntRegIndivState.PAGE_ADDR) or fuzzerstate.privilegestate.privstate in fuzzerstate.taint_sink_privs and fuzzerstate.intregpickstate.exists_reg_in_state(IntRegIndivState.PAGE_TAINT_ADDR)
        ...
        if USE_MMU:
            fuzzerstate.csrfile.regs[CSR_IDS.SEPC].unreliable = True
            fuzzerstate.csrfile.regs[CSR_IDS.SCAUSE].unreliable = True
        return SimpleExceptionEncapsulator_taint(fuzzerstate,is_mtvec,None,MisalignedMemInstruction_taint(fuzzerstate, is_mtvec, True),exception_op_type)
        ...
        if USE_MMU:
            fuzzerstate.csrfile.regs[CSR_IDS.SEPC].unreliable = True
            fuzzerstate.csrfile.regs[CSR_IDS.SCAUSE].unreliable = True
        return SimpleExceptionEncapsulator_taint(fuzzerstate,is_mtvec,None, MisalignedMemInstruction_taint(fuzzerstate, is_mtvec, False), exception_op_type)
        ...
        fuzzerstate.is_minstret_inaccurate_because_ecall_ebreak = ('rocket' in fuzzerstate.design_name and not is_tolerate_rocket_minstret()) # rocket has minstret inaccurate because of ecall/ebreak
        return SimpleExceptionEncapsulator_taint(fuzzerstate,is_mtvec, None, EcallEbreakInstruction(fuzzerstate,"ecall"),exception_op_type)
        ...
        instr_str = random.choice([i for i in IntLoadInstruction_taint.authorized_instr_strs if not i.startswith("c")])
        ...
        rs1 = fuzzerstate.intregpickstate.pick_int_reg_in_state(IntRegIndivState.PAGE_TAINT_ADDR)
        ...
        return SimpleExceptionEncapsulator_taint(fuzzerstate,is_mtvec, None, IntLoadInstruction_taint(fuzzerstate, instr_str, rd, rs1, imm, None, False),exception_op_type)
...
def gen_tvecfill_instr(fuzzerstate):
    ...
    if rd > 0:
        fuzzerstate.intregpickstate.set_regstate(rd, IntRegIndivState.RELOCUSED, force=True)
    return TvecWriterInstruction_taint(fuzzerstate,is_mtvec, rd, rs1, producer_id)
...
def gen_epcfill_instr(fuzzerstate):
    ...
    return EPCWriterInstruction_taint(fuzzerstate, is_mepc, rd, rs1, producer_id)
...
def gen_ppfill_instrs(fuzzerstate):
    ...
    if is_mpp:
        if target_privlvl == PrivilegeStateEnum.USER:
            ret = [CSRRegInstruction_taint(fuzzerstate,"csrrc", rd, MPP_BOTH_ENDIS_REGISTER_ID, CSR_IDS.MSTATUS, False, (False,None), PrivilegeStateEnum.USER)]
            ...
            ret =  [CSRRegInstruction_taint(fuzzerstate,"csrrs", rd, MPP_BOTH_ENDIS_REGISTER_ID, CSR_IDS.MSTATUS, False, (False,None), PrivilegeStateEnum.SUPERVISOR), 
                    CSRRegInstruction_taint(fuzzerstate,"csrrc", rd, MPP_TOP_ENDIS_REGISTER_ID, CSR_IDS.MSTATUS,False, (False,None), PrivilegeStateEnum.SUPERVISOR)]
        elif target_privlvl == PrivilegeStateEnum.MACHINE:
            ret = [CSRRegInstruction_taint(fuzzerstate,"csrrs", rd, MPP_BOTH_ENDIS_REGISTER_ID, CSR_IDS.MSTATUS,False, (False,None), PrivilegeStateEnum.MACHINE)]
        else:
            raise NotImplementedError("Hypervisor mode not implemented")
    else:
        if target_privlvl == PrivilegeStateEnum.USER:
            ret = [CSRRegInstruction_taint(fuzzerstate,"csrrc", rd, SPP_ENDIS_REGISTER_ID, CSR_IDS.MSTATUS, False, (False,None), PrivilegeStateEnum.USER)]
        elif target_privlvl == PrivilegeStateEnum.SUPERVISOR:
            ret = [CSRRegInstruction_taint(fuzzerstate,"csrrs", rd, SPP_ENDIS_REGISTER_ID, CSR_IDS.MSTATUS,False, (False,None), PrivilegeStateEnum.SUPERVISOR)]
        else:
...
def gen_medeleg_instr(fuzzerstate):
    ...
    return GenericCSRWriterInstruction_taint(fuzzerstate, CSR_IDS.MEDELEG, rd, rs1, producer_id, val_to_write_spike, val_to_write_cpu)
...
def gen_sum_mprv_op(fuzzerstate):
    ...
    return MstatusWriterInstruction_taint(0, rs1, producer_id, instr_str, mstatus_mask)