...
from params.runparams import DO_ASSERT, DO_EXPENSIVE_ASSERT, PRINT_FSM_TRANSITIONS, PRINT_WRITEBACK_TAINT
from params.fuzzparams import REGPICK_PROTUBERANCE_RATIO,  REGPICK_PROTUBERANCE_RATIO_TAINT_POS, REGPICK_PROTUBERANCE_RATIO_TAINT_NEG, NUM_MIN_FREE_INTREGS,  MAX_NUM_PICKABLE_REGS, NUM_MIN_UNTAINTED_INTREGS, MIN_WEIGHT_TAINT, MAX_WEIGHT_TAINT, P_TAINT_REG, NUM_MIN_TAINTED_REGS, DISABLE_COMPUTATION_ON_TAINT
...
from milesan.registers import IntRegister, INTREG_ABINAMES
from common.spike import SPIKE_STARTADDR, SPIKE_BOOTVAL_A1
from milesan.registers import INTREG_ABINAMES,MAX_32b, MAX_64b
...
class IntRegPickState:
    def __init__(self, fuzzerstate):
        ...
        self.__reg_weights_taint  = np.ones(self.num_pickable_regs)
        self.__reg_weights_taint /= np.sum(self.__reg_weights_taint)
    ...
    def set_initial_values(self, fuzzerstate): # Reset seed to starting value to ensure random values match if this function is called twice.
        ...
        for i,reg_data_content in enumerate(fuzzerstate.initial_reg_data_content):
            ...
            if random.random() < P_TAINT_REG:
                self.regs[i+1].set_val_taint(random.randint(1,MAX_64b if self.fuzzerstate.is_design_64bit else MAX_32b))
            else:
                self.regs[i+1].set_val_taint(0x0)
        self.regs[RELOCATOR_REGISTER_ID].set_val(SPIKE_STARTADDR)
        self.regs[RELOCATOR_REGISTER_ID].set_val_taint(0x0)
        self.regs[RDEP_MASK_REGISTER_ID].set_val(MAX_32b)
        self.regs[RDEP_MASK_REGISTER_ID].set_val_taint(0x0)

    def set_spike_boot_values(self): # Spike implicitly executes a couple of boot instructions that change the register values.
        if INTREG_ABINAMES.index("t0") in range(self.num_pickable_regs): # We only modify them if they are considered during the register checks.
            self.regs[INTREG_ABINAMES.index("t0")].set_val(SPIKE_STARTADDR) # The provided SPIKE_STARTADDR is loaded into t0 during boot.
        if INTREG_ABINAMES.index("a1") in range(self.num_pickable_regs):
            self.regs[INTREG_ABINAMES.index("a1")].set_val(SPIKE_BOOTVAL_A1)
    ...
    def get_untainted_regs_onehot(self):
        ret = [int(self.regs[reg_id].get_val_taint() == 0) for reg_id in range(self.num_pickable_regs)]
        return np.asarray(ret)
    ...
    def _get_rel_taint_hw(self): 
        return np.asarray([self.regs[reg_id].get_val_taint().bit_count()/self.regs[reg_id].n_bits for reg_id in range(self.num_pickable_regs)])

    def _get_taint_bin(self): 
        return np.asarray([self.regs[reg_id].get_val_taint().bit_count()>0 for reg_id in range(self.num_pickable_regs)])
    ...
    def _get_rel_taint_tanh(self): # this is nice because it saturates and thus we prefer more taint but dont overfit on fully tainted regs
        return np.asarray([np.tanh(self.regs[reg_id].get_val_taint().bit_count()*np.pi/self.regs[reg_id].n_bits) for reg_id in range(self.num_pickable_regs)])
    ...
    def _get_taint_ps(self, inverse: bool):
        ...
        if not inverse: # taint makes them more likely
            taint_ps = self.__reg_weights + taint_ps*REGPICK_PROTUBERANCE_RATIO_TAINT_POS # Add pertubation to drive probability up for registers with higher taint hamming weight.
            taint_ps = np.asarray([i if i<MAX_WEIGHT_TAINT else MAX_WEIGHT_TAINT for i in taint_ps]) # upper bound with MAX_WEIGHT_TAINT
        else: # taint makes them less likely
            taint_ps = self.__reg_weights - taint_ps*REGPICK_PROTUBERANCE_RATIO_TAINT_NEG # Substract to do the opposite.            
            taint_ps = np.asarray([i if i>MIN_WEIGHT_TAINT else MIN_WEIGHT_TAINT for i in taint_ps]) # lower bound with MIN_WEIGHT_TAINT
        return taint_ps

    # Weights after deducting the forbidden registers
    def get_effective_weights_taint(self, authorized_regs_onehot, inverse = False, force = False):
        if not force:
        ...
        if DO_ASSERT:
            if not force:
                states = {INTREG_ABINAMES[i]:[j,k,l] for i,(j,k,l) in enumerate(zip(self.__reg_weights,taint_ps,authorized_regs_onehot))}
            else:
                states = {INTREG_ABINAMES[i]:[j,k] for i,(j,k) in enumerate(zip(taint_ps,authorized_regs_onehot))}
        ...
        # print({INTREG_ABINAMES[i]:[j,k,l] for i,(j,k,l) in enumerate(zip(self.__reg_weights,taint_ps,authorized_regs_onehot))})
    ...
    def pick_tainted_int_inputreg(self, authorize_sideeffects: bool = True, force: bool = False, allow_zero: bool = True):
        ...
        id = random.choices(range(self.num_pickable_regs), self.get_effective_weights_taint(authorized_regs_onehot, False, force))[0]
        if DO_ASSERT:
    ...
    def pick_untainted_int_inputreg_nonzero(self, force: bool = False, val_range: tuple = None, val_taint_range: tuple = None):
        authorized_regs_onehot = self.get_free_regs_onehot()
        was_zero_authorized = authorized_regs_onehot[0]
        authorized_regs_onehot[0] = 0
        ...
        if val_taint_range is not None:
            assert TAINT_EN
            regs_in_range_onehot &= [reg.get_val_taint() in val_taint_range for reg in self.regs[:self.num_pickable_regs]]
        if DO_ASSERT:
        ...
        id = random.choices(range(self.num_pickable_regs), self.get_effective_weights_taint([i&j for i,j in zip(authorized_regs_onehot,regs_in_range_onehot)], True, force))[0]
        authorized_regs_onehot[0] = was_zero_authorized
        if DO_ASSERT:
            assert self.regs[id].fsm_state == IntRegIndivState.FREE
        return id

    def exists_free_intreg_in_range(self, val_range: tuple = None, val_taint_range: tuple = None, allow_zero: bool = False):
    ...
    def pick_untainted_int_inputreg(self, force: bool = False):
        authorized_regs_onehot = self.get_free_regs_onehot()
        id = random.choices(range(self.num_pickable_regs), self.get_effective_weights_taint(authorized_regs_onehot, True, force))[0]
        if DO_ASSERT:
            assert self.regs[id].fsm_state == IntRegIndivState.FREE
        return id
    ...
    def pick_tainted_int_inputregs(self, n: int, force: bool = False):
        ...
        if DO_ASSERT:
            assert n > 1, "The function pick_int_inputregs should not be used for n < 2. For n = 1, please use pick_int_inputreg."
        return random.choices(range(self.num_pickable_regs), self.get_effective_weights_taint(authorized_regs_onehot, False, force), k=n)
    ...
    def pick_untainted_int_inputregs(self, n: int, force: bool = False):
        ...
        if DO_ASSERT:
            assert n > 1, "The function pick_int_inputregs should not be used for n < 2. For n = 1, please use pick_int_inputreg."
        return random.choices(range(self.num_pickable_regs), self.get_effective_weights_taint(authorized_regs_onehot, True, force), k=n)
    ...
    def pick_untainted_int_outputreg(self, authorize_sideeffects: bool = True, force: bool = False):
        ...
        if DO_ASSERT:
            assert np.max(authorized_regs_onehot) == 1, "Unexpectedly, some register was registered in two states at a time."
        rd = random.choices(range(self.num_pickable_regs), self.get_effective_weights_taint(authorized_regs_onehot, True, force))[0]
        if authorize_sideeffects:
            self._update_probaweights(rd)
            if rd:
                self.set_regstate(rd, IntRegIndivState.FREE)
    ...
    def pick_tainted_int_outputreg(self, authorize_sideeffects: bool = True, force: bool = False, allow_zero: bool = True):
        ...
        if DO_ASSERT:
            assert np.max(authorized_regs_onehot) == 1, "Unexpectedly, some register was registered in two states at a time."
        rd = random.choices(range(self.num_pickable_regs), self.get_effective_weights_taint(authorized_regs_onehot, False, force))[0]
        if authorize_sideeffects:
            self._update_probaweights(rd)
            if rd:
                self.set_regstate(rd, IntRegIndivState.FREE)
    ...
    def pick_untainted_int_outputregs_nonzero(self, n:int, authorize_sideeffects: bool = True, force: bool = False):
        ...
        while regs is None or len(set(regs)) != n:
            regs = random.choices(range(self.num_pickable_regs), self.get_effective_weights_taint(authorized_regs_onehot, True, force), k=n)
        if authorize_sideeffects:
            for rd in regs:
                self._update_probaweights(rd)
                if rd:
    ...
    def set_regstate(self, reg_id: int, new_state: int, force: bool = False):
        ...
        if PRINT_FSM_TRANSITIONS:
            print(f"Setting {INTREG_ABINAMES[reg_id]}: {self.regs[reg_id].fsm_state.name} -> {new_state.name}")
    ...
    def exists_tainted_reg(self):
        return any([r.get_val_taint() for r in self.regs.values()])

    def get_tainted_free_regs(self):
        return [reg_id for reg_id in range(self.num_pickable_regs) if self.regs[reg_id].get_val_taint() and self.regs[reg_id].fsm_state == IntRegIndivState.FREE] 

    def get_untainted_free_regs(self):
        return [reg_id for reg_id in range(self.num_pickable_regs) if self.regs[reg_id].get_val_taint() == 0 and self.regs[reg_id].fsm_state == IntRegIndivState.FREE] 
    ...
    def pick_untainted_int_reg_in_state(self, req_state: IntRegIndivState, force: bool = False):
        ...
        untainted_regs_in_state = self.get_effective_weights_taint(self.__regs_in_state_onehot[req_state], True, force)
        ...
        if DO_ASSERT:
            ...
            if force:
                assert self.regs[ret].get_val_taint() == 0, f"Chosen register {INTREG_ABINAMES[ret]} is tainted! {untainted_regs_in_state}"
        # if self.regs[ret].get_val_taint() == 0:
    ...
    def print(self):
        row = ["ID","VALUE","VALUE_TAINT","STATE","PICKABLE"]
    ...
    def print_and_compare(self,regdumps_rtl):
        row = ["ID","VALUE (sim/rtl)","VALUE_TAINT (sim/rtl)"]
    ...
    def free_pageregs(self):
        ...
        while self.exists_reg_in_state(IntRegIndivState.PAGE_TAINT_ADDR):
            reg_id = self.pick_int_reg_in_state(IntRegIndivState.PAGE_TAINT_ADDR)
            self.set_regstate(reg_id, IntRegIndivState.FREE, force=True)
