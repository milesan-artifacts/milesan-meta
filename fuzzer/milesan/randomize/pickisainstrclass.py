...
from params.fuzzparams import MAX_NUM_STORE_LOCATIONS, TAINT_EN
...
def _get_isainstrclass_filtered_weights(fuzzerstate, curr_alloc_cursor):
    ...
    if (not fuzzerstate.authorize_privileges) or not ((fuzzerstate.privilegestate.privstate == PrivilegeStateEnum.MACHINE and not (fuzzerstate.privilegestate.is_mepc_populated and fuzzerstate.privilegestate.is_sepc_populated)) or \
        ...
        "picorv32" in fuzzerstate.design_name \
        or "vexriscv" in fuzzerstate.design_name and is_forbid_vexriscv_csrs() \
        or "kronos" in fuzzerstate.design_name and fuzzerstate.privilegestate.is_mepc_populated \
        or "ibex" in fuzzerstate.design_name: # does not work on ibex 
    ...
    if (not TAINT_EN or fuzzerstate.privilegestate.privstate not in fuzzerstate.taint_source_privs) and not fuzzerstate.intregpickstate.exists_reg_in_state(IntRegIndivState.PAGE_ADDR):
        ret_dict[ISAInstrClass.MEM] = 0
    if (not TAINT_EN or fuzzerstate.privilegestate.privstate in fuzzerstate.taint_source_privs and fuzzerstate.effective_curr_layout in fuzzerstate.taint_source_layouts) and not fuzzerstate.intregpickstate.exists_reg_in_state(IntRegIndivState.PAGE_TAINT_ADDR):
        ret_dict[ISAInstrClass.MEM] = 0
    # if fuzzerstate.effective_curr_layout in fuzzerstate.taint_source_layouts and not fuzzerstate.intregpickstate.exists_reg_in_state(IntRegIndivState.PAGE_TAINT_ADDR):
    #     ret_dict[ISAInstrClass.MEM] = 0
...
def _filter_taint(fuzzerstate, filtered_weights: list):
    if fuzzerstate.privilegestate.privstate in fuzzerstate.taint_source_privs and fuzzerstate.effective_curr_layout in fuzzerstate.taint_source_layouts and not DISABLE_COMPUTATION_ON_TAINT:
        ...
        if n_free_untainted_regs < NUM_MIN_UNTAINTED_INTREGS:
            ...
            if fuzzerstate.intregpickstate.exists_reg_in_state(IntRegIndivState.PAGE_TAINT_ADDR):
    ...
    elif not fuzzerstate.intregpickstate.exists_reg_in_state(IntRegIndivState.PAGE_ADDR): # also for PAGE_TAINT_ADDR?
        filtered_weights = dict.fromkeys(filtered_weights,0)
