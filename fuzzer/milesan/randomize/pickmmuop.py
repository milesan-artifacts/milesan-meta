...
from milesan.cfinstructionclasses_taint import R12DInstruction_taint, CSRRegInstruction_taint, ImmRdInstruction_taint, RegImmInstruction_taint, TvecWriterInstruction_taint, SpecialInstruction_taint, JALRInstruction_taint
...
def update_mmu_fsm_rv32(fuzzerstate, curr_addr):
    ...
    if fuzzerstate.curr_mmu_state == MmuState.IDLE:
        ...
        if target_layout == -1 and fuzzerstate.privilegestate.privstate != PrivilegeStateEnum.SUPERVISOR:
            ...
            return [CSRRegInstruction_taint(fuzzerstate,"csrrw", tmp, 0, CSR_IDS.SATP, is_satp_smode=is_satp_smode)]
        else:
            fuzzerstate.target_layout = target_layout
            if fuzzerstate.curr_mmu_state == MmuState.IDLE:
                if fuzzerstate.privilegestate.privstate == PrivilegeStateEnum.SUPERVISOR \
...
def gen_stvec_satp(fuzzerstate, curr_addr):
    ...
    instr_objs.append(ImmRdInstruction_taint(fuzzerstate,"lui", stvec_val_reg, lui_imm))
    instr_objs.append(RegImmInstruction_taint(fuzzerstate,"addi", stvec_val_reg, stvec_val_reg, addi_imm))
    if fuzzerstate.is_design_64bit and fuzzerstate.target_layout != -1:
        instr_objs.append(R12DInstruction_taint(fuzzerstate,"and", stvec_val_reg, stvec_val_reg, RPROD_MASK_REGISTER_ID))
    if fuzzerstate.is_design_64bit and fuzzerstate.target_layout == -1:
        instr_objs.append(R12DInstruction_taint(fuzzerstate,"and", stvec_val_reg, stvec_val_reg, RDEP_MASK_REGISTER_ID))
    instr_objs.append(TvecWriterInstruction_taint(fuzzerstate,False, stvec_val_reg, stvec_val_reg, -1))
...
def gen_satp_write(fuzzerstate, curr_addr):
    ...
    if fuzzerstate.is_design_64bit:
        ...
        instr_objs.append(ImmRdInstruction_taint(fuzzerstate,"lui", satp_val_reg, lui_imm))
        instr_objs.append(RegImmInstruction_taint(fuzzerstate,"addi", satp_val_reg, satp_val_reg, addi_imm))

    # Write to SATP
    is_satp_smode = ((fuzzerstate.privilegestate.privstate == PrivilegeStateEnum.SUPERVISOR), fuzzerstate.target_layout)
    instr_objs.append(CSRRegInstruction_taint(fuzzerstate, "csrrw", satp_val_reg, satp_val_reg, CSR_IDS.SATP, is_satp_smode=is_satp_smode))
    ...
    if (not is_from_bare) and old_asid == new_asid:
        ...
        instr_objs.append(SpecialInstruction_taint(fuzzerstate, "sfence.vma", 0, 0, 0))
        fuzzerstate.n_missing_r_cmds += 1
    ...
    # If the old layout was global, we also need an sfence, or the TLB will continue using the old mappings.
        ...
        instr_objs.append(SpecialInstruction_taint(fuzzerstate, "sfence.vma", 0, 0, 0)) #TODO we can flush only the old ASID, must have a reg for that
...
def gen_jump_new_layout(fuzzerstate, curr_addr):
    ...
    instr_objs.append(ImmRdInstruction_taint(fuzzerstate,"lui", tmp, lui_imm)) # Cannot produce, rely on freshly generated RPROD
    instr_objs.append(RegImmInstruction_taint(fuzzerstate,"addi", tmp, tmp, addi_imm))
    if fuzzerstate.is_design_64bit: 
        instr_objs.append(R12DInstruction_taint(fuzzerstate,"and", tmp, tmp, RPROD_MASK_REGISTER_ID))
    # Finally, we jump to the new layout
    instr_objs.append(JALRInstruction_taint(fuzzerstate,"jalr", tmp, tmp, 0, -1, True))
