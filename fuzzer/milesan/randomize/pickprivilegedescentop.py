...
from params.fuzzparams import USE_MMU, MAX_NUM_INSTR_IN_PRV, MIN_NUM_INSTR_IN_PRV, USE_MMU, TAINT_EN
from milesan.privilegestate import PrivilegeStateEnum
from milesan.cfinstructionclasses_taint import PrivilegeDescentInstruction_taint
...
def gen_priv_descent_instr(fuzzerstate):
    ...
    if TAINT_EN and fuzzerstate.privilegestate.privstate == PrivilegeStateEnum.MACHINE and fuzzerstate.privilegestate.curr_mstatus_mpp not in fuzzerstate.taint_source_privs:
        instr_objs += clear_taints_with_random_instructions(fuzzerstate, untaint_all=True)
    elif TAINT_EN and fuzzerstate.privilegestate.privstate == PrivilegeStateEnum.SUPERVISOR and PrivilegeStateEnum.SUPERVISOR in fuzzerstate.taint_source_privs and fuzzerstate.privilegestate.curr_mstatus_spp not in fuzzerstate.taint_source_privs:
        instr_objs += clear_taints_with_random_instructions(fuzzerstate, untaint_all=True)
    ...
    if USE_MMU:
        if fuzzerstate.privilegestate.privstate == PrivilegeStateEnum.MACHINE:
            ...
            if DEBUG_PRINT: 
                ...
                if TAINT_EN and fuzzerstate.real_curr_layout in fuzzerstate.taint_source_layouts: 
        ...
        if TAINT_EN and fuzzerstate.real_curr_layout not in fuzzerstate.taint_source_layouts: 
            instr_objs += clear_taints_with_random_instructions(fuzzerstate, untaint_all=True)
    ...
    instr_objs += [PrivilegeDescentInstruction_taint(fuzzerstate, is_mret)]
