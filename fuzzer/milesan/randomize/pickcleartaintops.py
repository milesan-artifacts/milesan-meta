...
from milesan.cfinstructionclasses_taint import R12DInstruction_taint, RegImmInstruction_taint, ImmRdInstruction_taint
...
def clear_taints_with_random_instructions(fuzzerstate, untaint_all: bool = False):
    ...
    for tainted_reg_id in tainted_reg_ids:
        ...
        del curr_param_size
        if instr_str in R12DInstruction_taint.authorized_instr_strs:
            rs1 = random.choice(untainted_reg_ids)
            rs2 = random.choice(untainted_reg_ids)
            instr_objs += [R12DInstruction_taint(fuzzerstate, instr_str, tainted_reg_id, rs1, rs2)]
        elif instr_str in RegImmInstruction_taint.authorized_instr_strs:
            rs1 = random.choice(untainted_reg_ids)
            instr_objs += [RegImmInstruction_taint(fuzzerstate, instr_str, tainted_reg_id, rs1, imm)]
        elif instr_str in ImmRdInstruction_taint.authorized_instr_strs:
            instr_objs += [ImmRdInstruction_taint(fuzzerstate, instr_str, tainted_reg_id, imm)]
        else:
