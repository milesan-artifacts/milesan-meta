...
from params.runparams import CHECK_MEM_TAINT_PRECISE, PRINT_MEM_STORES, PRINT_MEM_STORES_TAINT, PRINT_MEM_LOADS, PRINT_MEM_LOADS_TAINT, INSERT_REGDUMPS
...
class MemoryView:
    ...
    def __init__(self, fuzzerstate):
        ...
        self.data_taint = {} # Keep track of load/store operations' taints. Holds the addr-byte_taint pairs in little endian format.
        self.states = []
    ...
    def gen_random_page_addr_from_randomblocks(self, max_attempts: int = MEMVIEW_ALLOC_MAX_ATTEMPTS, tainted: bool = False):
        for _ in range(max_attempts):
            ...
            if TAINT_EN:
                ...
                allowed_pages = [page_start_end_addr[0] for page_start_end_addr in allowed_pages]
            assert len(allowed_pages), f"No pages matched required criteria: tainted: {tainted}"
    ...
    def is_addr_tainted(self,addr,n_bytes):
        ...
        for i in range(n_bytes):
            is_tainted |= addr+i in self.data_taint and self.data_taint[addr+i]
        return is_tainted
    ...
    def read_taint(self, addr, n_bytes: int = 4, priv_level: PrivilegeStateEnum = PrivilegeStateEnum.MACHINE, va_layout: int = -1):
        if USE_MMU:
            addr = virt2phys(addr, priv_level, va_layout, self.fuzzerstate,absolute_addr=False)
        if DO_ASSERT:
            try:
        ...
        val_taint = 0
        for i in range(n_bytes):
            assert addr+i in self.data_taint, f"Taint read request from invalid address {hex(addr+i)}."
            b = self.data_taint[addr+i]
            assert b <= 0xFF
            val_taint |= (b << (i*8))
        if PRINT_MEM_LOADS_TAINT:
            print(f"TAINT: Reading {n_bytes} bytes {hex(val_taint)} from {hex(addr)}")
        if DO_ASSERT:
            assert not USE_MMU or va_layout == -1 or priv_level in self.fuzzerstate.taint_source_privs or val_taint == 0, f"Write from unauthorized privilege: {priv_level.name}, allowed are {self.fuzzerstate.taint_source_privs}"
            assert not USE_MMU or va_layout == -1  or va_layout in self.fuzzerstate.taint_source_layouts or val_taint == 0, f"Write from unmapped layout: {va_layout}, allowed are {self.fuzzerstate.taint_source_layouts}"
        return val_taint
    ...
    def write(self, addr, val, n_bytes, priv_level: PrivilegeStateEnum = PrivilegeStateEnum.MACHINE, va_layout: int = -1):
        ...
        for i in range(n_bytes):
            ...
            if addr+i not in self.data_taint:
                self.data_taint[addr+i] = 0

    def write_taint(self, addr, val_taint, n_bytes, priv_level: PrivilegeStateEnum = PrivilegeStateEnum.MACHINE, va_layout: int = -1):
        if USE_MMU:
            addr = virt2phys(addr, priv_level, va_layout, self.fuzzerstate,absolute_addr=False)
        if DO_ASSERT:
            try:
        ...
        if PRINT_MEM_STORES_TAINT: 
            print(f"TAINT: Writing {n_bytes} bytes {hex(val_taint)} to {hex(addr)}")
        for i in range(n_bytes):
            b = (val_taint&(0xFF<<(i*8)))>>(i*8)
            # print(f"Writing to {hex(addr+i)}: {hex(b)}")
            self.data_taint[addr+i] = b # little endian
        ...
        for i,reg_data_content in enumerate(fuzzerstate.initial_reg_data_content):
            ...
            if TAINT_EN and n_tainted_regs < MAX_NUM_INIT_TAINTED_REGS:
                if random.random() < P_TAINT_REG:
                    ...
                    n_tainted_regs += 1
                    self.write_taint(addr, rand_val, n_bytes)
            else:
                self.write_taint(addr, 0, n_bytes)

    def store_state(self): 
        self.states += [(deepcopy(self.data), deepcopy(self.data_taint))]  # deepcopy important so we can restore and execute multiple times

    def set_as_initial_state(self):
        self.states[0] = (deepcopy(self.data), deepcopy(self.data_taint))
    ...
    def restore_and_reduce_taint(self, mismatch):
        ...
        if mismatch:
            ...
            h_ = hash(frozenset(self.data_taint.items()))
        ...
        for addr, val_taint in self.data.items():
            if val_taint:
                for i in range(8): # access at byte-granularity
                    if (self.data_taint[addr]>>i)&1 and np.random.choice([1,0],p=[P_UNTAINT_BIT, 1-P_UNTAINT_BIT]):
                        self.data_taint[addr] &= ~(1<<i) # untained single bits
                # break
        n_total_tainted_bits = 0
        for addr, val_taint in self.data.items():
            n_total_tainted_bits += val_taint.bit_count()
        
        self.states.append((deepcopy(self.data), deepcopy(self.data_taint)))
    ...
    def restore(self, bb_id: int = -1):# TODO: the index wont correspond to bb_id as we store before that already?
        ...
        self.data_taint = deepcopy(self.states[bb_id][1])

    def reset(self):
        self.data.clear()
        self.data_taint.clear()

    def dump_taint(self, path):
        dumped_waddrs = []
        wlen = 8 if self.fuzzerstate.is_design_64bit else 4
        with open(path, "w") as f:
            for addr in self.data_taint.keys():
                if DO_ASSERT:
                    assert addr >= SPIKE_STARTADDR
                    assert addr < SPIKE_STARTADDR + self.fuzzerstate.memsize
                if self.data_taint[addr]:
                    waddr = addr - addr % wlen
                    if waddr not in dumped_waddrs:
                        dumped_waddrs += [waddr]
                        wordstring = "".join(["{:02x}".format(self.data_taint[waddr+i]) if waddr+i in self.data_taint else "00" for i in range(wlen)]) 
                        f.write("0 {:x} {:x} ".format(waddr, wlen) + wordstring + "\n")

    def print(self):
        row = ["ADDRESS","VALUE","VALUE_TAINT"]
        print("{: >30} {: >30} {: >30}".format(*row))
        row = ["*"*30,"*"*30,"*"*30]
        print("{: >30} {: >30} {: >30}".format(*row))
        addresses = self.data_taint.keys()
        printed_addresses = []
        for addr in addresses:
            ...
            for i in range(8 if self.fuzzerstate.is_design_64bit else 4):
                if addr+i in self.data:
                    assert addr+i in self.data_taint
                    val |= self.data[addr+i]<<(i*8)
                    val_taint |= self.data_taint[addr+i]<<(i*8)
                    printed_addresses += [addr+i]
            row = ["0x{:08x}".format(addr), "0x{:08x}".format(val), "0x{:08x}".format(val_taint)]
            print("{: >30} {: >30} {: >30}".format(*row))
    

    def print_and_compare(self,rtl_values):
        row = ["ADDRESS","VALUE (sim/rtl)","VALUE_TAINT (sim/rtl)"]
        print("{: >30} {: >30} {: >30}".format(*row))
        row = ["*"*30,"*"*30,"*"*30]
        print("{: >30} {: >30} {: >30}".format(*row))
        addresses = self.data_taint.keys()
        ...
        for addr in addresses:
            ...
            if addr in rtl_values:
                ...
                rtl_val_taint = rtl_values[addr]["val_taint"]
            else:
                rtl_val = None
                rtl_val_taint = None
            if rtl_val == val:
            ...
            if rtl_val_taint == val_taint:
                val_taint_str =  "0x{:08x}".format(val_taint)
            else:
                val_taint_str = "0x{:08x} != 0x{:08x}".format(val_taint,rtl_val_taint) if rtl_val_taint is not None else "0x{:08x} (NONE)".format(val_taint)

            row = ["0x{:08x}".format(addr), val_str,val_taint_str]
            print("{: >30} {: >30} {: >30}".format(*row))


    def check(self, rtl_values, precise = CHECK_MEM_TAINT_PRECISE):
        addresses = self.data_taint.keys()
        checked_addresses = []
        ...
        for addr in addresses:
            ...
            assert addr in rtl_values or val_taint == 0, f"Address {hex(addr)} not found."
            if addr in rtl_values:
                rtl_val = rtl_values[addr]["val"]
                rtl_val_taint = rtl_values[addr]["val_taint"]

                mismatch_val = rtl_val != val
                mismatch_val_taint = rtl_val_taint != val_taint if precise else ~val_taint&rtl_val_taint != 0
            
                assert not mismatch_val, f"Value mismatch at address {hex(addr)}: {hex(val)} != {hex(rtl_val)}"
                if TAINT_EN:
                    assert not mismatch_val_taint, f"Taint mismatch at address {hex(addr)}: {hex(val_taint)} != {hex(rtl_val_taint)}"
        if TAINT_EN:
            for addr in rtl_values.keys():
                assert addr in addresses or rtl_values[addr]["val_taint"] == 0 or addr in [regdump_addr,fpregdump_addr] , f"Memory at untracked address {hex(addr)} tainted in RTL simulation: {hex(rtl_values[addr]['val_taint'])}/{hex(rtl_values[addr]['val'])} (val_taint/val)."

    def flip_tainted_bits(self):
        assert TAINT_EN
        for addr,val_taint in self.data_taint.items():
            if val_taint:
                self.data[addr] ^= val_taint
