...
def run_rtl_and_load_regstream(fuzzerstate): 
    assert not COV_EN or fuzzerstate.simulator == SimulatorEnum.MODELSIM, f"Coverage only in modelsim right now."
    designstr = "vanilla" if not TAINT_EN else "drfuzz_mem"
    tracestr = "trace" if TRACE_EN else "trace_fst" if TRACE_FST else "notrace"
    simstr = "" if fuzzerstate.simulator == SimulatorEnum.VERILATOR else "_modelsim"
    cmd = [
        "make",
        f"rerun_{designstr}_{tracestr}{simstr}"
        ]

    design_name = fuzzerstate.design_name
    milesandir = designcfgs.get_design_milesan_path(design_name)
    env = os.environ
    assert "SIMSRAMELF" in fuzzerstate.env
    assert "SIMSRAMTAINT" in fuzzerstate.env
    assert "SIMLEN" in fuzzerstate.env
    assert "REGDUMP_PATH" in fuzzerstate.env
    assert "REGSTREAM_PATH" in fuzzerstate.env

    env.update(fuzzerstate.env)

    if(os.path.exists(env["REGDUMP_PATH"])):
        os.remove(env["REGDUMP_PATH"])

    if(os.path.exists(env["REGSTREAM_PATH"])):
        os.remove(env["REGSTREAM_PATH"])
    ...
    if CHECK_MEM and not USE_VANILLA:
        ...
        with open(env["SRAMDUMP_PATH"], "r") as f:
            for line in f.read().split("\n"):
                ...
                sramdump_rtl[addr]["val_taint"] = int(d["value_t0"],16)
    return (regstream_rtl_val, regstream_rtl_val_taint), (regdump_rtl_val, regdump_rtl_val_taint), sramdump_rtl, None
