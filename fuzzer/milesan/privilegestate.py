...
from params.fuzzparams import USE_MMU, ALLOW_LOAD_PAGE_FAULT_IN_TAINT_SINK_PRIVS, ALLOW_LOAD_PAGE_FAULT_IN_TAINT_SOURCE_PRIVS, TAINT_EN
...
class PrivilegeState:
    ...
    def gen_takable_exception_dict(self, fuzzerstate):
        ...
        has_page_reg = fuzzerstate.intregpickstate.exists_reg_in_state(IntRegIndivState.PAGE_ADDR) or fuzzerstate.intregpickstate.exists_reg_in_state(IntRegIndivState.PAGE_TAINT_ADDR)
        if not has_page_reg:
        ...
        if TAINT_EN and self.privstate in fuzzerstate.taint_source_privs and not ALLOW_LOAD_PAGE_FAULT_IN_TAINT_SOURCE_PRIVS:
            supported_exceptions_dict[ExceptionCauseVal.ID_LOAD_PAGE_FAULT] = False

        if TAINT_EN and self.privstate in fuzzerstate.taint_sink_privs and not ALLOW_LOAD_PAGE_FAULT_IN_TAINT_SINK_PRIVS:
            supported_exceptions_dict[ExceptionCauseVal.ID_LOAD_PAGE_FAULT] = False
