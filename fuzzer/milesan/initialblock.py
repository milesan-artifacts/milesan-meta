...
from milesan.cfinstructionclasses_taint import  ImmRdInstruction_taint, RegImmInstruction_taint, R12DInstruction_taint, IntLoadInstruction_taint, CSRRegInstruction_taint
...
def gen_initial_basic_block(fuzzerstate, offset_addr: int, csr_init_rounding_mode: int = 0):
    ...
    curr_addr += fuzzerstate.append_and_execute_instr(ImmRdInstruction_taint(fuzzerstate,"lui", RELOCATOR_REGISTER_ID, lui_imm, is_rd_nonpickable_ok=True), insert_regdump = False)
    curr_addr += fuzzerstate.append_and_execute_instr(RegImmInstruction_taint(fuzzerstate,"addi", RELOCATOR_REGISTER_ID, RELOCATOR_REGISTER_ID, addi_imm, is_rd_nonpickable_ok=True), insert_regdump = False)
    ...
    if INSERT_REGDUMPS:
        ...
        curr_addr += fuzzerstate.append_and_execute_instr(ImmRdInstruction_taint(fuzzerstate,"lui", REGDUMP_REGISTER_ID, lui_imm, is_rd_nonpickable_ok=True),insert_regdump = False)

        curr_addr += fuzzerstate.append_and_execute_instr(RegImmInstruction_taint(fuzzerstate,"addi", REGDUMP_REGISTER_ID, REGDUMP_REGISTER_ID, addi_imm, is_rd_nonpickable_ok=True),insert_regdump = False)
        
        if fuzzerstate.design_name in ["kronos"]: # For kronos we need to add the spike offset, for e.g. rocket not.
            curr_addr += fuzzerstate.append_and_execute_instr(R12DInstruction_taint(fuzzerstate,"add", REGDUMP_REGISTER_ID, REGDUMP_REGISTER_ID, RELOCATOR_REGISTER_ID, is_rd_nonpickable_ok=True),insert_regdump = False)

    if fuzzerstate.is_design_64bit:
        # Clear the top 32 bits
        curr_addr += fuzzerstate.append_and_execute_instr(RegImmInstruction_taint(fuzzerstate,"slli", RELOCATOR_REGISTER_ID, RELOCATOR_REGISTER_ID, 32, is_rd_nonpickable_ok=True),insert_regdump = False)
        
        curr_addr += fuzzerstate.append_and_execute_instr(RegImmInstruction_taint(fuzzerstate,"srli", RELOCATOR_REGISTER_ID, RELOCATOR_REGISTER_ID, 32, is_rd_nonpickable_ok=True),insert_regdump = False)
        
    if DO_ASSERT:
        assert curr_addr == fuzzerstate.curr_bb_start_addr + len(fuzzerstate.instr_objs_seq[-1]) * 4 # NO_COMPRESSED

    if not ("vexriscv" in fuzzerstate.design_name and is_forbid_vexriscv_csrs() or "ibex" in fuzzerstate.design_name):
        ...
        if fuzzerstate.design_has_supervisor_mode:
            curr_addr += fuzzerstate.append_and_execute_instr(CSRRegInstruction_taint(fuzzerstate,"csrrw", 0, 0, CSR_IDS.MEDELEG), insert_regdump = False)
        ...
        if fuzzerstate.design_name != 'picorv32':
            curr_addr += fuzzerstate.append_and_execute_instr(CSRRegInstruction_taint(fuzzerstate,"csrrw", 0, 0, CSR_IDS.MTVEC), insert_regdump = False)
            
        if fuzzerstate.design_has_supervisor_mode:
            curr_addr += fuzzerstate.append_and_execute_instr(CSRRegInstruction_taint(fuzzerstate,"csrrw", 0, 0, CSR_IDS.STVEC), insert_regdump = False)
    ...
    if not ("vexriscv" in fuzzerstate.design_name and is_forbid_vexriscv_csrs() or "ibex" in fuzzerstate.design_name):
        if fuzzerstate.design_has_pmp:
            # pmpcfg0
            curr_addr += fuzzerstate.append_and_execute_instr(RegImmInstruction_taint(fuzzerstate,"addi", 1, 0, 31),insert_regdump = False)
            
            curr_addr += fuzzerstate.append_and_execute_instr(CSRRegInstruction_taint(fuzzerstate,"csrrw", 0, 1, CSR_IDS.PMPCFG0), insert_regdump = False)
            
            # pmpaddr0
            if fuzzerstate.is_design_64bit:
                curr_addr += fuzzerstate.append_and_execute_instr(RegImmInstruction_taint(fuzzerstate,"addi", 1, 0, 1),insert_regdump = False)
                
                curr_addr += fuzzerstate.append_and_execute_instr(RegImmInstruction_taint(fuzzerstate,"slli", 1, 0, 0x36),insert_regdump = False)
                
                curr_addr += fuzzerstate.append_and_execute_instr(RegImmInstruction_taint(fuzzerstate,"addi", 1, 1, -1),insert_regdump = False)
                
                curr_addr += fuzzerstate.append_and_execute_instr(CSRRegInstruction_taint(fuzzerstate,"csrrw", 0, 1, CSR_IDS.PMPADDR0), insert_regdump = False)          
    ...
    if not ("vexriscv" in fuzzerstate.design_name and is_forbid_vexriscv_csrs()):
        if  'picorv32' not in fuzzerstate.design_name and 'ibex' not in fuzzerstate.design_name:
            curr_addr += fuzzerstate.append_and_execute_instr(CSRRegInstruction_taint(fuzzerstate,"csrrw", 0, 0, CSR_IDS.MCYCLE), insert_regdump = False)
            
            curr_addr += fuzzerstate.append_and_execute_instr(CSRRegInstruction_taint(fuzzerstate,"csrrw", 0, 0, CSR_IDS.MINSTRET), insert_regdump = False)
            
            curr_addr += fuzzerstate.append_and_execute_instr(CSRRegInstruction_taint(fuzzerstate,"csrrw", 0, 0, CSR_IDS.MCAUSE), insert_regdump = False)
            
            curr_addr += fuzzerstate.append_and_execute_instr(CSRRegInstruction_taint(fuzzerstate,"csrrw", 0, 0, CSR_IDS.MTVAL), insert_regdump = False)
            
            curr_addr += fuzzerstate.append_and_execute_instr(CSRRegInstruction_taint(fuzzerstate,"csrrw", 0, 0, CSR_IDS.MSCRATCH), insert_regdump = False)
            
        if fuzzerstate.design_has_supervisor_mode:
            curr_addr += fuzzerstate.append_and_execute_instr(CSRRegInstruction_taint(fuzzerstate,"csrrw", 0, 0, CSR_IDS.SCAUSE), insert_regdump = False)
            
            curr_addr += fuzzerstate.append_and_execute_instr(CSRRegInstruction_taint(fuzzerstate,"csrrw", 0, 0, CSR_IDS.STVAL), insert_regdump = False)
            
            curr_addr += fuzzerstate.append_and_execute_instr(CSRRegInstruction_taint(fuzzerstate,"csrrw", 0, 0, CSR_IDS.SSCRATCH), insert_regdump = False)
            

        if not fuzzerstate.is_design_64bit and fuzzerstate.design_name != 'picorv32':
            curr_addr += fuzzerstate.append_and_execute_instr(CSRRegInstruction_taint(fuzzerstate,"csrrw", 0, 0, CSR_IDS.MCYCLEH), insert_regdump = False)
            
            curr_addr += fuzzerstate.append_and_execute_instr(CSRRegInstruction_taint(fuzzerstate,"csrrw", 0, 0, CSR_IDS.MINSTRETH), insert_regdump = False)
            
    # Prepare FPU_ENDIS_REGISTER_ID, which will be used across the program's execution, also required to setup MPP
    curr_addr += fuzzerstate.append_and_execute_instr(ImmRdInstruction_taint(fuzzerstate,"lui", FPU_ENDIS_REGISTER_ID, 0b10, is_rd_nonpickable_ok=True),insert_regdump = False)
    ...
    if fuzzerstate.design_has_fpu:
        ...
        # Enable the FPU
        curr_addr += fuzzerstate.append_and_execute_instr(CSRRegInstruction_taint(fuzzerstate,"csrrw", 0, FPU_ENDIS_REGISTER_ID, CSR_IDS.MSTATUS), insert_regdump = False)
        ...
        curr_addr += fuzzerstate.append_and_execute_instr(RegImmInstruction_taint(fuzzerstate,"addi", 1, 0, 0),insert_regdump = False)
        
        curr_addr += fuzzerstate.append_and_execute_instr(CSRRegInstruction_taint(fuzzerstate,"csrrw", 0, 1, CSR_IDS.FCSR), insert_regdump = False)
        

    if fuzzerstate.design_has_supervisor_mode or fuzzerstate.design_has_user_mode:
        curr_addr += fuzzerstate.append_and_execute_instr(RegImmInstruction_taint(fuzzerstate,"srli", MPP_TOP_ENDIS_REGISTER_ID, FPU_ENDIS_REGISTER_ID, 1, is_rd_nonpickable_ok=True),insert_regdump = False)
        
        curr_addr += fuzzerstate.append_and_execute_instr(RegImmInstruction_taint(fuzzerstate,"srli", MPP_BOTH_ENDIS_REGISTER_ID, FPU_ENDIS_REGISTER_ID, 2, is_rd_nonpickable_ok=True),insert_regdump = False)
        
        curr_addr += fuzzerstate.append_and_execute_instr(R12DInstruction_taint(fuzzerstate,"or", MPP_BOTH_ENDIS_REGISTER_ID, MPP_BOTH_ENDIS_REGISTER_ID, MPP_TOP_ENDIS_REGISTER_ID,is_rd_nonpickable_ok=True),insert_regdump = False)
        
        if INIT_MIE:
            curr_addr += fuzzerstate.append_and_execute_instr(CSRRegInstruction_taint(fuzzerstate,"csrrw", 0, 0, CSR_IDS.MIE), insert_regdump = False)
        else: # Just for the alignment.
            curr_addr += fuzzerstate.append_and_execute_instr(RegImmInstruction_taint(fuzzerstate,"addi", 0, 0, 0),insert_regdump = False)
        ...
        curr_addr += fuzzerstate.append_and_execute_instr(CSRRegInstruction_taint(fuzzerstate,"csrrs", 0, MPP_BOTH_ENDIS_REGISTER_ID, CSR_IDS.MSTATUS), insert_regdump = False)
        
        curr_addr += fuzzerstate.append_and_execute_instr(CSRRegInstruction_taint(fuzzerstate,"csrrs", 0, MPP_TOP_ENDIS_REGISTER_ID, CSR_IDS.MSTATUS), insert_regdump = False)
        

    if not ("vexriscv" in fuzzerstate.design_name and is_forbid_vexriscv_csrs() or "ibex" in fuzzerstate.design_name):
        if fuzzerstate.design_has_user_mode:
            curr_addr += fuzzerstate.append_and_execute_instr(RegImmInstruction_taint(fuzzerstate,"srli", SPP_ENDIS_REGISTER_ID, FPU_ENDIS_REGISTER_ID, 5, is_rd_nonpickable_ok=True),insert_regdump = False)
             # NO_COMPRESSED
            # While it is not necesary to set the mpp initially, it is convenient to do so. If we don't, then we should adapt the initial values (typically to None) in privilegestate.py
            curr_addr += fuzzerstate.append_and_execute_instr(CSRRegInstruction_taint(fuzzerstate,"csrrs", 0, SPP_ENDIS_REGISTER_ID, CSR_IDS.MSTATUS), insert_regdump = False)
             # NO_COMPRESSED

    # Set the rdep mask to the correct value

    if fuzzerstate.is_design_64bit:
        curr_addr += fuzzerstate.append_and_execute_instr(RegImmInstruction_taint(fuzzerstate,"addi", RDEP_MASK_REGISTER_ID, 0, -1, is_rd_nonpickable_ok=True),insert_regdump = False)
        
        curr_addr += fuzzerstate.append_and_execute_instr(RegImmInstruction_taint(fuzzerstate,"slli", RDEP_MASK_REGISTER_ID, RDEP_MASK_REGISTER_ID, 32, is_rd_nonpickable_ok=True),insert_regdump = False)
        
        curr_addr += fuzzerstate.append_and_execute_instr(RegImmInstruction_taint(fuzzerstate,"xori", RDEP_MASK_REGISTER_ID, RDEP_MASK_REGISTER_ID, -1, is_rd_nonpickable_ok=True),insert_regdump = False)
    ...
    curr_addr += fuzzerstate.append_and_execute_instr(R12DInstruction_taint(fuzzerstate,"add", fuzzerstate.num_pickable_regs-1, 0, RELOCATOR_REGISTER_ID),insert_regdump = False)
    

    if USE_MMU:
        curr_addr += fuzzerstate.append_and_execute_instr(R12DInstruction_taint(fuzzerstate,"sub", RDEP_MASK_REGISTER_ID_VIRT, RDEP_MASK_REGISTER_ID, RELOCATOR_REGISTER_ID, is_rd_nonpickable_ok=True))
        curr_addr += fuzzerstate.append_and_execute_instr(CSRRegInstruction_taint(fuzzerstate, "csrrw", 0, 0, CSR_IDS.SATP)) # init satp to 0
    ...
    next_instr = RegImmInstruction_taint(fuzzerstate,"addi", fuzzerstate.num_pickable_regs-1, fuzzerstate.num_pickable_regs-1, bytes_until_random_vals + curr_addr)
    ...
    for reg_id in range(1, fuzzerstate.num_pickable_regs):
        next_instr = IntLoadInstruction_taint(fuzzerstate,"ld" if fuzzerstate.is_design_64bit else "lw", reg_id, fuzzerstate.num_pickable_regs-1, 8*(reg_id-1), -1)
        curr_addr += fuzzerstate.append_and_execute_instr(next_instr, insert_regdump = False)
