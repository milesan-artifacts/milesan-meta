...
from rv.asmutil import li_into_reg, twos_complement, to_unsigned, INSTR_FUNCS, INSTR_FUNCS_TAINT
...
from milesan.registers import INTREG_ABINAMES, MAX_32b, MAX_64b, MAX_20b
    ...
    for bb_instrs in fuzzerstate.instr_objs_seq:
        for instr_obj in bb_instrs:
            ...
            if (instr_obj.vaddr if USE_MMU else instr_obj.paddr) == addr: # reached this instruction
                assert last_instr is not None, f"Traceback computation for instruction at {hex(addr)} failed: No previous instruction modifying register {INTREG_ABINAMES[reg_id]} with mismatch {hex(fuzzerstate.intregpickstate.regs[reg_id].get_val())} =! {hex(correct_val)} found."
    ...
    def check_regs(self,reg_cmp):
        for reg_id,reg_val in reg_cmp.items():
            if reg_id not in self.fuzzerstate.intregpickstate.regs:
                if PRINT_CHECK_REGS:
                    print(f"{self.get_preamble()}: Ignoring register value: {INTREG_ABINAMES[reg_id]}")
                continue
            if PRINT_CHECK_REGS:
                print(f"{self.get_preamble()}: Checking register value: {INTREG_ABINAMES[reg_id]}:{hex(reg_val)}")
            mismatch = self.fuzzerstate.intregpickstate.regs[reg_id].check(reg_val)
            assert not mismatch, f"{self.get_str()}: (Inst) Value mismatch for {INTREG_ABINAMES[reg_id]}: {hex(mismatch[1])} != {hex(reg_val)}\n\t Traceback: {compute_reg_traceback(reg_id,self.paddr,self.fuzzerstate,reg_val).get_str()}"
...
class R12DInstruction(CFInstruction):
    ...
    def get_str(self, is_spike_resolution: bool = USE_SPIKE_INTERM_ELF, color_taint: bool = False):
        return f"{self.get_preamble()}: {self.instr_str} {INTREG_ABINAMES[self.rd]}, {INTREG_ABINAMES[self.rs1]}, {INTREG_ABINAMES[self.rs2]}"
...
class ImmRdInstruction(ImmInstruction):
    ...
    def get_str(self, is_spike_resolution: bool = USE_SPIKE_INTERM_ELF, color_taint: bool = False):
        return f"{self.get_preamble()}: {self.instr_str} {INTREG_ABINAMES[self.rd]}, {hex(self.imm)}"
...
class RegImmInstruction(ImmInstruction):
    ...
    def get_str(self, is_spike_resolution: bool = USE_SPIKE_INTERM_ELF, color_taint: bool = False):
        return f"{self.get_preamble()}: {self.instr_str} {INTREG_ABINAMES[self.rd]}, {INTREG_ABINAMES[self.rs1]}, {hex(self.imm)}"
...
class BranchInstruction(ImmInstruction):
    ...
    def get_str(self, is_spike_resolution: bool = USE_SPIKE_INTERM_ELF, color_taint: bool = False):
        return f"{self.get_preamble()}: {self.instr_str} {INTREG_ABINAMES[self.rs1]}, {INTREG_ABINAMES[self.rs2]}, {hex(self.imm)}"
...
class SpecialInstruction(CFInstruction):
    ...
    def get_str(self, is_spike_resolution: bool = USE_SPIKE_INTERM_ELF, color_taint: bool = False):
        return f"{self.get_preamble()}: {self.instr_str} {INTREG_ABINAMES[self.rd]}, {INTREG_ABINAMES[self.rs1]}"
...
class IntLoadInstruction(ImmInstruction):
    ...
    def get_str(self, is_spike_resolution: bool = USE_SPIKE_INTERM_ELF, color_taint: bool = False):
        return f"{self.get_preamble()}: {self.instr_str} {INTREG_ABINAMES[self.rd]}, {self.imm}({INTREG_ABINAMES[self.rs1]}) "
...
class CSRRegInstruction(CSRInstruction):
    ...
    def get_str(self, is_spike_resolution: bool = True, color_taint: bool = False):
        return f"{self.get_preamble()}: {self.instr_str} {INTREG_ABINAMES[self.rd]}, {self.csr_id.name}, {INTREG_ABINAMES[self.rs1]}"
...
class CSRImmInstruction(CSRInstruction):
    ...
    def get_str(self, is_spike_resolution: bool = USE_SPIKE_INTERM_ELF, color_taint: bool = False):
        return f"{self.get_preamble()}: {self.instr_str} {INTREG_ABINAMES[self.rd]}, {self.csr_id.name}, {hex(self.uimm)}"
    ...
    def get_str(self, is_spike_resolution: bool = USE_SPIKE_INTERM_ELF, color_taint: bool = False):
        if is_spike_resolution:
            if self.spike_resolution_offset is not None:
                ...
                return  f"{self.get_preamble()}: {self.instr_str} {INTREG_ABINAMES[self.rd]}, {hex(li_into_reg(to_unsigned(spike_res_off, self.fuzzerstate.is_design_64bit), False)[0])}"
            return f"{self.get_preamble()}: {self.instr_str} {INTREG_ABINAMES[self.rd]}, [undetermined]"
        else:
            if self.rtl_offset is not None:
                ...
                return f"{self.get_preamble()}: {self.instr_str} {INTREG_ABINAMES[self.rd]}, {hex(li_into_reg(to_unsigned(rtl_off, self.fuzzerstate.is_design_64bit), False)[0])}"
            else:
                return f"{self.get_preamble()}: {self.instr_str} {INTREG_ABINAMES[self.rd]}, (None)"
    ...
    def get_str(self, is_spike_resolution: bool = USE_SPIKE_INTERM_ELF, color_taint: bool = False):
        if is_spike_resolution:
            if self.spike_resolution_offset is not None:
                ...
                return f"{self.get_preamble()}: {self.instr_str} {INTREG_ABINAMES[self.rd]}, {INTREG_ABINAMES[self.rd]}, {hex(li_into_reg(to_unsigned(spike_res_off, self.fuzzerstate.is_design_64bit), False)[1])}"
            return f"{self.get_preamble()}: {self.instr_str} {INTREG_ABINAMES[self.rd]}, {INTREG_ABINAMES[self.rd]}, [undetermined]"
        else:
            if self.rtl_offset is not None:
                rtl_off = self.rtl_offset
                if USE_MMU and self.fuzzerstate.is_design_64bit and self.produce_va_layout != -1: 
                    rtl_off = (self.rtl_offset | 0x80000000) & 0xffffffff # TODO double check if the check of the 64th bit is valid
                return f"{self.get_preamble()}: {self.instr_str} {INTREG_ABINAMES[self.rd]}, {INTREG_ABINAMES[self.rd]}, {hex(li_into_reg(to_unsigned(rtl_off, self.fuzzerstate.is_design_64bit), False)[1])}"
            else:
                return f"{self.get_preamble()}: {self.instr_str} {INTREG_ABINAMES[self.rd]}, {INTREG_ABINAMES[self.rd]}, (None)"
...
class PlaceholderPreConsumerInstr(BaseInstruction):
    ...
    def get_str(self, is_spike_resolution: bool = False, color_taint: bool = False):
        if USE_MMU and self.fuzzerstate.is_design_64bit and self.is_rprod and self.produce_va_layout != -1:
            return f"{self.get_preamble()}: {self.instr_str} {INTREG_ABINAMES[self.rdep]},  {INTREG_ABINAMES[self.rdep]}, {INTREG_ABINAMES[RPROD_MASK_REGISTER_ID]}"
        elif USE_MMU and self.fuzzerstate.is_design_64bit and self.produce_va_layout != -1:
            return f"{self.get_preamble()}: {self.instr_str} {INTREG_ABINAMES[self.rdep]},  {INTREG_ABINAMES[self.rdep]}, {INTREG_ABINAMES[RDEP_MASK_REGISTER_ID_VIRT]}"
        else:
            return f"{self.get_preamble()}: {self.instr_str} {INTREG_ABINAMES[self.rdep]},  {INTREG_ABINAMES[self.rdep]}, {INTREG_ABINAMES[RDEP_MASK_REGISTER_ID]}"
    ...
    def get_str(self, is_spike_resolution: bool = False, color_taint: bool = False):
        if is_spike_resolution:
            if USE_MMU and self.produce_va_layout != -1:
                ...
                return f"{self.get_preamble()}: {self.instr_str} {INTREG_ABINAMES[self.rd]}, {INTREG_ABINAMES[self.rprod]}, {INTREG_ABINAMES[RELOCATOR_REGISTER_ID]}"
        else:
            return f"{self.get_preamble()}: {self.instr_str} {INTREG_ABINAMES[self.rd]}, {INTREG_ABINAMES[self.rdep]}, {INTREG_ABINAMES[self.rprod]}"
...
class MisalignedMemInstruction(ExceptionInstruction):
    ...
    def __init__(self,fuzzerstate, is_mtvec: bool, is_load: bool, iscompressed: bool = False):
        ...
        if self.priv_level in fuzzerstate.taint_sink_privs:
            if DO_ASSERT:
                assert fuzzerstate.intregpickstate.exists_reg_in_state(IntRegIndivState.PAGE_ADDR) or fuzzerstate.intregpickstate.exists_reg_in_state(IntRegIndivState.PAGE_TAINT_ADDR)
            if not fuzzerstate.intregpickstate.exists_reg_in_state(IntRegIndivState.PAGE_ADDR):
                rs1 = fuzzerstate.intregpickstate.pick_int_reg_in_state(IntRegIndivState.PAGE_ADDR_TAINT)
            elif not  fuzzerstate.intregpickstate.exists_reg_in_state(IntRegIndivState.PAGE_ADDR_TAINT):
                rs1 = fuzzerstate.intregpickstate.pick_int_reg_in_state(IntRegIndivState.PAGE_ADDR)
            else:
                rs1 = fuzzerstate.intregpickstate.pick_int_reg_in_state(IntRegIndivState.PAGE_ADDR if random.random()<0.5 else IntRegIndivState.PAGE_ADDR_TAINT)
        else:
            if DO_ASSERT:
